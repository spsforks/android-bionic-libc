...
static inline
void __arm64_mul96(uint64_t a, uint64_t b, uint64_t* h, uint64_t* l) {
    uint128_t re = (uint128_t)a * b;
    *l = re;
    *h = re >> 64;
}
#else
...
static inline
void __arm64_mulaccum96(uint64_t x, uint64_t y, uint64_t* h, uint64_t* l)
{
    uint64_t o1, o2, ch= *h, cl= *l;
    __arm64_mul96(x, y, &o2, &o1);
...
uint32_t __arm64_rem_pio2(uint64_t x, double *y)
{
    ...
    if (shiftup) {
        ...
        __arm64_mulaccum96(mant, word, &c1, &c2);
