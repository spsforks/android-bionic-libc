...
# -----------------------------------------------------------------------------
# libm_common.a: Main math code.
# -----------------------------------------------------------------------------
include $(CLEAR_VARS)

LOCAL_MODULE := libm_common
...
LOCAL_C_INCLUDES_x86 += $(LOCAL_PATH)/x86
...
LOCAL_ARM_MODE := arm
# weak_symbols.h marks all exported symbols as weak so that they
# can be overridden by arch specific optimized routines.
LOCAL_CFLAGS := \
    $(libm_common_cflags) \
    -DFLT_EVAL_METHOD=0 \
    -include $(LOCAL_PATH)/weak-symbols.h \
...
# -----------------------------------------------------------------------------
# libm_override.a: Functions that override code from libm_common.a
# -----------------------------------------------------------------------------
include $(CLEAR_VARS)

LOCAL_MODULE := libm_override

include $(LOCAL_PATH)/arch/$(TARGET_ARCH)/$(TARGET_ARCH).mk
LOCAL_ADDITIONAL_DEPENDENCIES += $(LOCAL_PATH)/arch/$(TARGET_ARCH)/$(TARGET_ARCH).mk
ifdef TARGET_2ND_ARCH
include $(LOCAL_PATH)/arch/$(TARGET_2ND_ARCH)/$(TARGET_2ND_ARCH).mk
LOCAL_ADDITIONAL_DEPENDENCIES += $(LOCAL_PATH)/arch/$(TARGET_2ND_ARCH)/$(TARGET_2ND_ARCH).mk
endif
...
LOCAL_CLANG := $(libm_clang)
LOCAL_ADDITIONAL_DEPENDENCIES := $(LOCAL_PATH)/Android.mk
# libm_override must go first. It allows code to override the weak
# symbols found in libm_common.
...
# -----------------------------------------------------------------------------
# libm_test_overrides.a: Test code to override functions from libm_common.
# -----------------------------------------------------------------------------
include $(CLEAR_VARS)

LOCAL_MODULE := libm_test_overrides
LOCAL_CLANG := $(libm_clang)
LOCAL_SRC_FILES := test/overrides.cpp
LOCAL_ADDITIONAL_DEPENDENCIES := $(LOCAL_PATH)/Android.mk
LOCAL_CFLAGS := $(libm_common_cflags)
...
# -----------------------------------------------------------------------------
# libm_test_with_overrides.a: Test library containing libm_test_overrides and
#                             libm_common code.
# -----------------------------------------------------------------------------
include $(CLEAR_VARS)

LOCAL_MODULE := libm_test_with_overrides
LOCAL_CLANG := $(libm_clang)
LOCAL_ADDITIONAL_DEPENDENCIES := $(LOCAL_PATH)/Android.mk
# libm_test must go first. It allows code to override the weak
# symbols found in libm_common.
LOCAL_WHOLE_STATIC_LIBRARIES := \
    libm_test_overrides \
    libm_common \
...
# -----------------------------------------------------------------------------
# libm_test_with_overrides.so: Test shared library containing
#                              libm_test_with_overrides.a.
# -----------------------------------------------------------------------------
include $(CLEAR_VARS)
...
LOCAL_ADDITIONAL_DEPENDENCIES := $(LOCAL_PATH)/Android.mk
LOCAL_SYSTEM_SHARED_LIBRARIES := libc
LOCAL_WHOLE_STATIC_LIBRARIES := libm_test_with_overrides
...
# -----------------------------------------------------------------------------
# libm_static_test_override: Gtest executable that uses the static test library
#                            to verify the overrides work.
# -----------------------------------------------------------------------------
include $(CLEAR_VARS)

LOCAL_MODULE := libm_static_test_override
LOCAL_CLANG := $(libm_clang)
LOCAL_SRC_FILES := test/override_test.cpp
LOCAL_CFLAGS := -fno-builtin
LOCAL_ADDITIONAL_DEPENDENCIES := $(LOCAL_PATH)/Android.mk
LOCAL_STATIC_LIBRARIES := libm_test_with_overrides
LOCAL_SYSTEM_SHARED_LIBRARIES := libc
...
# -----------------------------------------------------------------------------
# libm_shared_test_override: Gtest executable that uses the shared test library
#                            to verify the overrides work.
# -----------------------------------------------------------------------------
include $(CLEAR_VARS)

LOCAL_MODULE := libm_shared_test_override
LOCAL_CLANG := $(libm_clang)
LOCAL_SRC_FILES := test/override_test.cpp
LOCAL_CFLAGS := -fno-builtin
LOCAL_ADDITIONAL_DEPENDENCIES := $(LOCAL_PATH)/Android.mk
LOCAL_SHARED_LIBRARIES := libm_test_with_overrides
LOCAL_SYSTEM_SHARED_LIBRARIES := libc
