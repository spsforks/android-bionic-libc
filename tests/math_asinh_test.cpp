...
static asinh_intel_data_t g_asinh_intel_data[] = {
  { // Entry 0
...
TEST(math_asinh, asinh_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_asinh_intel_data)/sizeof(asinh_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_asinh_intel_data[i].expected, asinh(g_asinh_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
