...
#define MAX_AL      64
#define NUM_AL      10
// Use only first 4 values for wchar.
#define WNUM_AL     4
...
// Calculate input lengths and fill len with them.
...
// reference result (what we expect of function under test) and
// then run function and compare what we got.


// For str* and some mem* functions we test lengths less or equal 1024.
#define MAX_LEN     1024
  ...
  for (i = 1; i < n; i++)
      ...
      for (j = 0; j < POS_ITER; j++)
	{
	  ...
	  if (strchr ((char *) ptr1, seek_char) != ref)
	    {
	      printf
		("ITER: %d\tlen: %d\talign1: %d\tseek_char: %d\tpos: %u\n", i,
		 len[i], align1, seek_char, pos);
	      error++;
	    }
	}
    }
...
test_strcmp (int n)
{
  ...
  for (i = 1; i < n; i++)
      ...
      for (j = 0; j < POS_ITER; j++)
	{
	  ...
	  if ((r == 0 && ref) || (r < 0 && ref >= 0) || (r > 0 && ref <= 0))
	    {
  ...
  for (i = 0; i < n; i++)
      ...
      for (j = 0; j < POS_ITER; j++)
	{
	  ...
	  if (memcmp (ptr, ptr2, MAX_LEN + len[i]) != 0)
	    {
	      printf
		("ITER: %d\tlen: %d\t, align1: %d\talign2: %d\tpos: %d\n", i,
		 len[i], align1, align2, pos);
	      error++;
	    }
	}
    }
  ...
  for (i = 1; i < n; i++)
      ...
      for (j = 0; j < POS_ITER; j++)
	{
	  ...
	  if (memcmp (ptr, ptr2, 2 * MAX_LEN) != 0)
	    {
	      printf
		("ITER: %d\tlen: %d\talign1: %d\talign2: %d\tpos: %d\tr: %d\n",
  ...
  for (i = 0; i < n; i++)
      ...
      for (j = 0; j < POS_ITER; j++)
	{
	  ...
	  if (memchr (ptr1, seek_char, len[i]) != ref)
	    {
	      printf
		("ITER: %d\tlen: %d\talign1: %d\tpos: %d\tseek_char: %u\n", i,
		 len[i], align1, pos, seek_char);
	      error++;
	    }
	}
    }
  ...
  for (i = 0; i < n; i++)
      ...
      for (j = 0; j < POS_ITER; j++)
	{
	  ...
	  if (memrchr (ptr1, seek_char, len[i]) != ref)
	    {
	      printf
		("ITER: %d\tlen: %d\talign1: %d\tpos: %d\tseek_char: %d\n", i,
		 len[i], align1, pos, seek_char);
	      error++;
	    }
	}
  ...
  for (i = 0; i < n; i++)
      ...
      for (j = 0; j < POS_ITER; j++)
	{
	  ...
	  if ((r == 0 && ref) || (r < 0 && ref >= 0) || (r > 0 && ref <= 0))
	    {
	      printf
		("ITER: %d\tlen:%d\talign1: %d\talign2: %d\treturned: %d\texpeted: %d\n",
...
// For widechar consider its size.
#undef MAX_LEN
...
// Functions below should be tested for lengths up to 2Mb.
#undef MAX_LEN
