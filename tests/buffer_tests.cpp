...
static const size_t g_single_aligns_len =
    sizeof(g_single_aligns) / sizeof(int[2]);
...
static const size_t g_double_aligns_len =
    sizeof(g_double_aligns) / sizeof(int[4]);
...
void RunSingleBufferAlignTest(size_t max_test_size,
                              void (*test_func)(uint8_t*, size_t),
  ...
  for (size_t i = 0; i < g_single_aligns_len; i++) {
    ...
    for (size_t len = 0; len <= max_test_size; len += incr) {
      ...
      buf_align = reinterpret_cast<uint8_t*>(
          GetAlignedPtr(buf + FENCEPOST_LENGTH, g_single_aligns[i][0],
                        g_single_aligns[i][1]));
...
void RunSrcDstBufferAlignTest(size_t max_test_size,
                              void (*test_func)(uint8_t*, uint8_t*, size_t),
                              size_t (*set_incr)(size_t)) {
  if (!set_incr) {
    set_incr = SetIncrement;
  }
  ...
  for (size_t i = 0; i < g_double_aligns_len; i++) {
    ...
    for (size_t len = 0; len <= max_test_size; len += incr) {
      ...
      src_align = reinterpret_cast<uint8_t*>(
          GetAlignedPtr(src + FENCEPOST_LENGTH, g_double_aligns[i][0],
                        g_double_aligns[i][1]));
      dst_align = reinterpret_cast<uint8_t*>(
          GetAlignedPtr(dst + FENCEPOST_LENGTH, g_double_aligns[i][2],
                        g_double_aligns[i][3]));
...
void RunCmpBufferAlignTest(size_t max_test_size,
                           void (*test_cmp_func)(uint8_t*, uint8_t*, size_t),
                           void (*test_miscmp_func)(uint8_t*, uint8_t*, size_t,
                                                    size_t),
                           size_t (*set_incr)(size_t)) {
  if (!set_incr) {
    set_incr = SetIncrement;
  ...
  for (size_t i = 0; i < g_double_aligns_len; i++) {
    ...
    for (size_t len = 0; len <= max_test_size; len += incr) {
      ...
      buf1_align = reinterpret_cast<uint8_t*>(
          GetAlignedPtr(buf1, g_double_aligns[i][0], g_double_aligns[i][1]));
      buf2_align = reinterpret_cast<uint8_t*>(
          GetAlignedPtr(buf2, g_double_aligns[i][2], g_double_aligns[i][3]));
...
void RunSingleBufferOverreadTest(void (*test_func)(uint8_t*, size_t)) {
  ...
  ASSERT_TRUE(mprotect(&memory[pagesize], pagesize, PROT_READ | PROT_WRITE) ==
              0);
  free(memory);
}

void RunSrcDstBufferOverreadTest(void (*test_func)(uint8_t*, uint8_t*,
                                                   size_t)) {
  // In order to verify that functions are not reading past the end of the
  // src, create data that ends exactly at an unreadable memory boundary.
  size_t pagesize = static_cast<size_t>(sysconf(_SC_PAGE_SIZE));
...
void RunCmpBufferOverreadTest(void (*test_cmp_func)(uint8_t*, uint8_t*, size_t),
                              void (*test_miscmp_func)(uint8_t*, uint8_t*,
                                                       size_t, size_t)) {
  ...
  ASSERT_TRUE(mprotect(&memory1[pagesize], pagesize, PROT_READ | PROT_WRITE) ==
              0);
  ASSERT_TRUE(mprotect(&memory2[pagesize], pagesize, PROT_READ | PROT_WRITE) ==
              0);
  free(memory1);
  free(memory2);
}
