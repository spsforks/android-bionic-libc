...
void RunSingleBufferAlignTest(size_t max_test_size, void (*test_func)(uint8_t*, size_t),
  ...
  for (size_t i = 0; i < g_single_aligns_len; i++) {
    ...
    for (size_t len = 0; len <= max_test_size; len += incr) {
      ...
      buf_align = reinterpret_cast<uint8_t*>(
          GetAlignedPtr(buf + FENCEPOST_LENGTH, g_single_aligns[i][0], g_single_aligns[i][1]));
...
void RunSrcDstBufferAlignTest(size_t max_test_size, void (*test_func)(uint8_t*, uint8_t*, size_t),
                              size_t (*set_incr)(size_t)) {
  if (!set_incr) {
    set_incr = SetIncrement;
  }
  ...
  for (size_t i = 0; i < g_double_aligns_len; i++) {
    ...
    for (size_t len = 0; len <= max_test_size; len += incr) {
      ...
      src_align = reinterpret_cast<uint8_t*>(
          GetAlignedPtr(src + FENCEPOST_LENGTH, g_double_aligns[i][0], g_double_aligns[i][1]));
      dst_align = reinterpret_cast<uint8_t*>(
          GetAlignedPtr(dst + FENCEPOST_LENGTH, g_double_aligns[i][2], g_double_aligns[i][3]));
...
void RunCmpBufferAlignTest(size_t max_test_size, void (*test_cmp_func)(uint8_t*, uint8_t*, size_t),
...
void RunCmpBufferOverreadTest(void (*test_cmp_func)(uint8_t*, uint8_t*, size_t),
  ...
  uint8_t* memory2;
  ASSERT_TRUE(posix_memalign(reinterpret_cast<void**>(&memory2), pagesize, 2 * pagesize) == 0);
