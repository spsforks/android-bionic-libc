...
static atan_intel_data_t g_atan_intel_data[] = {
  { // Entry 0
...
TEST(math_atan, atan_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_atan_intel_data)/sizeof(atan_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_atan_intel_data[i].expected, atan(g_atan_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
