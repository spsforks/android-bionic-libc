...
class TestCase {
 public:
  ...
  void AppendTest(const std::string& test_name) {
    test_list_.push_back(std::make_tuple(test_name, TestFailed, 0LL));
  }
  ...
  int64_t GetElapsedTime() {
    if (elapsed_time_ == 0) {
      ...
      for (size_t i = 0; i < test_list_.size(); ++i) {
        total_time += std::get<2>(test_list_[i]);
      }
...
void TestResultPrinter::OnTestStart(const ::testing::TestInfo& test_info) {
...
static void ListTest(std::vector<TestCase> &testcase_list) {
  for (size_t i = 0; i < testcase_list.size(); ++i) {
    ...
    for (int j = 0; j < testcase.NumTests(); ++j) {
...
static void OnTestIterationStartPrint(std::vector<TestCase>& testcase_list, int iteration,
  ...
  int test_num = 0;
  for (size_t i = 0; i < testcase_list.size(); ++i) {
    test_num += testcase_list[i].NumTests();
  }
...
static void OnTestIterationEndPrint(std::vector<TestCase>& testcase_list, int /*iteration*/,
  ...
  for (size_t i = 0; i < testcase_list.size(); ++i) {
    ...
    for (int j = 0; j < testcase.NumTests(); ++j) {
      ...
      if (result == TestSuccess) {
        ...
        ++timeout_test_num;
        timeout_test_list.push_back(std::make_pair(testcase.GetTestName(j),
                                                   testcase.GetTestTime(j)));
      }
    }
  }
  ...
  if (timeout_test_num > 0) {
    ...
    for (size_t i = 0; i < timeout_test_list.size(); ++i) {
      ColoredPrintf(COLOR_RED, "[ TIMEOUT  ] ");
      printf("%s (stopped at %lld ms)\n", timeout_test_list[i].first.c_str(),
                                          timeout_test_list[i].second / 1000000LL);
    }
  }
    ...
    while (finish_testcase_num < static_cast<int>(testcase_list.size())) {
      ...
      for (size_t i = 0; i < child_proc_list.size(); ++i) {
        if (child_proc_list[i].pid != 0 && child_proc_list[i].done_flag == true) {
          ...
          if (++finish_test_num_list[testcase_id] == testcase_list[testcase_id].NumTests()) {
