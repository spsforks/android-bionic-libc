...
static void PrintHelpInfo() {
  printf("Bionic Unit Test Options:\n"
         ...
         "      It implies enable of --isolate.\n"
         "      If JOB_NUM is not specified, it is set to the number of cpus.\n"
         "\n");
}
...
static bool PickOptions(int* pargc, char*** pargv, bool* exit_flag) {
  ...
  *exit_flag = false;
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    // If running in isolate mode, main process will not call testing::InitGoogleTest(&argc, argv).
