...
struct ChildProcInfo {
  ...
  int64_t end_time_ns;
  int64_t deadline_time_ns; // The time when the test is thought of as timeout.
  ...
  void Reset() {
    pid = 0;
    start_time_ns = end_time_ns = deadline_time_ns = 0;
    testcase_id = test_id = 0;
    done_flag = timeout_flag = false;
    exit_status = 0;
    child_read_fd = 0;
  }
};
...
static void RunChildProcess(const std::string& test_name, int testcase_id, int test_id,
                            sigset_t sigmask, int argc, char** argv, ChildProcInfo& child_proc) {
  int pipefd[2];
  ...
  // In Parent process, initialize child test info.
  close(pipefd[1]);
  child_proc.child_read_fd = pipefd[0];
  child_proc.pid = pid;
  child_proc.start_time_ns = NanoTime();
  child_proc.deadline_time_ns = child_proc.start_time_ns + GetDeadlineInfo(test_name) * 1000000LL;
...
static size_t CheckChildProcTimeout(std::vector<ChildProcInfo>& child_proc_list) {
  ...
  for (size_t i = 0; i < child_proc_list.size(); ++i) {
    if (child_proc_list[i].pid != 0 && child_proc_list[i].deadline_time_ns <= current_time_ns) {
      child_proc_list[i].done_flag = true;
...
static void WaitChildProcs(std::vector<TestCase>& testcase_list,
  ...
  while (true) {
    ...
    if (result == -1) {
      if (errno == ECHILD) {
        // It happens when We have no running child processes.
        return;
      } else {
        perror("waitpid");
        exit(1);
      ...
      finish_child_count += CheckChildProcTimeout(child_proc_list);
    }

    if (finish_child_count > 0) {
      return;
    }
...
static void CollectChildTestResult(const ChildProcInfo& child_proc, TestCase& testcase) {
  ...
  while (true) {
    ...
    if (ret > 0) {
      ...
      if (errno == EAGAIN) {
        // Read end as we mark O_NONBLOCK when opening the pipe.
        // This rarely happens when the child process created other processes which have not exited so far.
        break;
      }
    ...
    while (finished_testcase_count < testcase_list.size()) {
      ...
      for (auto& child_proc : child_proc_list) {
        if (child_proc.pid == 0 && next_testcase_id < testcase_list.size()) {
          ...
          RunChildProcess(test_name, next_testcase_id, next_test_id, orig_mask,
                          argc, argv, child_proc);
