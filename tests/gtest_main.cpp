...
static void InsertArg(int* pargc, char** argv, int insert_pos, const char* arg) {
...
static void RemoveArg(int* pargc, char** argv, int remove_pos) {
...
static bool PickOptions(int* pargc, char** argv, bool* exit_flag) {
  int argc = *pargc;
  *exit_flag = false;
  for (int i = 0; i < argc; ++i) {
  ...
  int num_jobs_option = GetProcessorCount();
  ...
  for (int i = 1; i < argc; ++i) {
    if (strcmp(argv[i], "-j") == 0) {
      ...
      if (i + 1 < argc && (num_jobs_option = atoi(argv[i + 1])) > 0) {
        RemoveArg(&argc, argv, i);
      } else {
  ...
  *pargc = argc;
  ...
  if (isolate_option) {
    *exit_flag = true;
    ...
    RunTestInSeparateProc(argc, argv, testcase_list, gtest_repeat_count, num_jobs_option);
    return true;
  }
  return true;
}
...
int main(int argc, char** argv) {
  // Allocate bigger argv to simplify following changes to argv.
