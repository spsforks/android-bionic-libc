...
class Test {
 public:
  ...
  void SetTestTime(int64_t elapsed_time) { elapsed_time_ = elapsed_time; }

  int64_t GetTestTime() const { return elapsed_time_; }

  void AppendFailTraceInfo(const std::string& s) { fail_trace_info_ += s; }

  const std::string& GetFailTraceInfo() const { return fail_trace_info_; }

 private:
  const std::string name_;
  TestResult result_;
  int64_t elapsed_time_;
  std::string fail_trace_info_;
};
...
class TestCase {
 public:
  ...
  void SetTestTime(size_t test_id, int64_t elapsed_time) {
    VerifyTestId(test_id);
    test_list_[test_id].SetTestTime(elapsed_time);
  }
...
// This is the file descriptor used by child process to write fail information.
// Parent process will collect the information and dump to stdout / xml file.
...
void TestResultPrinter::OnTestPartResult(const testing::TestPartResult& result) {
  ...
  char* p = buf;
  while (towrite > 0) {
    ssize_t write_count = write(child_output_fd, p, towrite);
    if (write_count == -1) {
      if (errno != EINTR) {
...
static void OnTestEndPrint(const TestCase& testcase, size_t test_id) {
  ...
  // Print trace info after test result.
  const std::string& traceinfo = testcase.GetTest(test_id).GetFailTraceInfo();
  if (traceinfo != "") {
    printf("%s", traceinfo.c_str());
  }
  fflush(stdout);
}
...
static void OnTestIterationEndPrint(const std::vector<TestCase>& testcase_list, size_t /*iteration*/,
  ...
  printf("%zu %s from %zu %s ran.", test_count, (test_count == 1) ? "test" : "tests",
                                    testcase_count, (testcase_count == 1) ? "test case" : "test cases");
  if (testing::GTEST_FLAG(print_time)) {
    printf(" (%" PRId64 " ms total)", elapsed_time / 1000000);
  }
  printf("\n");
...
// gtest.cc:XmlUnitTestResultPrinter.
void OnTestIterationEndXmlPrint(const std::string& xml_output_file,
  ...
  FILE* fp = fopen(xml_output_file.c_str(), "w");
  if (fp == NULL) {
    fprintf(stderr, "can't open xml_output_file: %s\n", xml_output_file.c_str());
    exit(1);
  }
  ...
  for (size_t i = 0; i < testcase_list.size(); ++i) {
    ...
    for (size_t j = 0; j < testcase.TestCount(); ++j) {
      ...
      if (testcase.GetTestResult(j) == TEST_SUCCESS) {
        ...
        fputs(">\n", fp);
        const std::string& traceinfo = testcase.GetTest(j).GetFailTraceInfo();
...
static void RunTestInSeparateProc(int argc, char** argv, std::vector<TestCase>& testcase_list,
                                  ...
                                  const std::string& xml_output_file) {
  ...
  for (size_t iteration = 1; iteration <= iteration_count; ++iteration) {
    ...
    while (finished_testcase_count < testcase_list.size()) {
      ...
      for (auto& child_proc : child_proc_list) {
        if (child_proc.pid != 0 && child_proc.done_flag == true) {
          ...
          // Kill and wait the timeout child process before we read trace info.
          ...
          int ret;
          while (true) {
            ret = read(child_proc.child_read_fd, buf, sizeof(buf) - 1);
            if (ret > 0) {
              buf[ret] = '\0';
              testcase.GetTest(test_id).AppendFailTraceInfo(buf);
            } else if (ret == 0) {
              break; // Read end.
            } else {
              if (errno == EINTR) {
    ...
    OnTestIterationEndXmlPrint(xml_output_file, testcase_list, epoch_iteration_start_time,
                               NanoTime() - iteration_start_time);
  }
}
...
struct IsolationTestOption {
  bool isolate;
  size_t job_count;
  int test_deadline_in_ms;
  int test_warnline_in_ms;
...
// gtest. PickOptions() picks the first part into IsolationTestOption structure, lefting the second
// part in args.
// Arguments:
//   args is used to pass in all command arguments, and pass out only the part of options for gtest.
//   option is used to pass out test options in isolation mode.
// Return false if there is error in arguments.
static bool PickOptions(std::vector<char*>& args, IsolationTestOption& option) {
  for (size_t i = 1; i < args.size(); ++i) {
    if (strcmp(args[i], "--help") == 0 || strcmp(args[i], "-h") == 0) {
      PrintHelpInfo();
      option.isolate = false;
      return true;
    }
  }
  ...
  if (option.isolate == false) {
    return true;
  }

  // Init default isolation test options.
  option.job_count = GetProcessorCount();
  option.test_deadline_in_ms = DEFAULT_GLOBAL_TEST_RUN_DEADLINE_IN_MS;
  option.test_warnline_in_ms = DEFAULT_GLOBAL_TEST_RUN_WARNLINE_IN_MS;
  option.gtest_color = testing::GTEST_FLAG(color);
  option.gtest_print_time = testing::GTEST_FLAG(print_time);
  option.gtest_repeat = testing::GTEST_FLAG(repeat);
  option.gtest_output = testing::GTEST_FLAG(output);
  ...
  for (size_t i = 1; i < args.size(); ++i) {
    if (strncmp(args[i], "-j", strlen("-j")) == 0) {
      ...
      if (count <= 0) {
        fprintf(stderr, "arguments about job_count is wrong: %d\n", count);
        return false;
      }
      option.job_count = static_cast<size_t>(count);
    } else if (strncmp(args[i], "--deadline=", strlen("--deadline=")) == 0) {
      int time_ms = atoi(args[i] + strlen("--deadline="));
      if (time_ms <= 0) {
        fprintf(stderr, "argument about deadline is wrong: %d\n", time_ms);
        return false;
      }
      option.test_deadline_in_ms = time_ms;
    } else if (strncmp(args[i], "--warnline=", strlen("--warnline=")) == 0) {
      int time_ms = atoi(args[i] + strlen("--warnline="));
      if (time_ms <= 0) {
        fprintf(stderr, "argument about warnline is wrong: %d\n", time_ms);
        return false;
      }
      option.test_warnline_in_ms = time_ms;
    } else if (strncmp(args[i], "--gtest_color=", strlen("--gtest_color=")) == 0) {
      option.gtest_color = args[i] + strlen("--gtest_color=");
    } else if (strcmp(args[i], "--gtest_print_time=0") == 0) {
      option.gtest_print_time = false;
    } else if (strncmp(args[i], "--gtest_repeat=", strlen("--gtest_repeat=")) == 0) {
      int repeat = atoi(args[i] + strlen("--gtest_repeat="));
      if (repeat < 0) {
        fprintf(stderr, "argument about repeat is wrong: %d\n", repeat);
        return false;
      }
      option.gtest_repeat = repeat;
      ...
      if (success) {
        option.gtest_output = output;
      } else {
        fprintf(stderr, "argument about output file is wrong: %s\n", args[i]);
        return false;
      }
...
int main(int argc, char** argv) {
  ...
  IsolationTestOption option;
  if (PickOptions(arg_list, option) == false) {
    return 1;
  }

  if (option.isolate == true) {
    // Set global variables.
    global_test_run_deadline_in_ms = option.test_deadline_in_ms;
    global_test_run_warnline_in_ms = option.test_warnline_in_ms;
    testing::GTEST_FLAG(color) = option.gtest_color.c_str();
    testing::GTEST_FLAG(print_time) = option.gtest_print_time;
    ...
    RunTestInSeparateProc(argc, arg_list.data(), testcase_list, option.gtest_repeat,
                          option.job_count, option.gtest_output);
  } else {
    argc = static_cast<int>(arg_list.size());
    arg_list.push_back(NULL);
