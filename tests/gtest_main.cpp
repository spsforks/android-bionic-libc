...
static void HandlingSignals(std::vector<TestCase>& testcase_list,
  ...
  while (true) {
    ...
    if (signo == -1) {
      if (errno == EAGAIN) {
        break; // Timeout, no pending signal.
      }
      perror("sigtimedwait");
      exit(1);
...
static void WaitChildProcs(std::vector<TestCase>& testcase_list,
  ...
  while (true) {
    int status;
    pid_t result = TEMP_FAILURE_RETRY(waitpid(-1, &status, WNOHANG));
    ...
    HandlingSignals(testcase_list, child_proc_list);
