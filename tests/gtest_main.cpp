...
static void OnTestEndPrint(const TestCase& testcase, size_t test_id) {
  ...
  // Print failure message after test result if it is not empty.
...
void OnTestIterationEndXmlPrint(const std::string& xml_output_filename,
                                ...
                                int64_t elapsed_time) {
  // If --gtest_output argument is not used, filename is empty and we don't need to write xml file.
...
static bool PickOptions(std::vector<char*>& args, IsolationTestOptions& options) {
  ...
  options.test_deadline_in_ms = DEFAULT_GLOBAL_TEST_RUN_DEADLINE_IN_MS;
  options.test_warnline_in_ms = DEFAULT_GLOBAL_TEST_RUN_WARNLINE_IN_MS;
  ...
  for (size_t i = 1; i < args.size(); ++i) {
    if (strncmp(args[i], "-j", strlen("-j")) == 0) {
      ...
      options.test_deadline_in_ms = time_ms;
      ...
      options.test_warnline_in_ms = time_ms;
...
int main(int argc, char** argv) {
  ...
  if (options.isolate == true) {
    ...
    global_test_run_deadline_in_ms = options.test_deadline_in_ms;
    global_test_run_warnline_in_ms = options.test_warnline_in_ms;
