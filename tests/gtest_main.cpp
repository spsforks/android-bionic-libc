...
static void OnTestTerminatedPrint(const TestCase& testcase, int test_id, int sig) {
  ColoredPrintf(COLOR_RED, "[  FAILED  ] ");
  ...
  for (size_t iteration = 1; iteration <= iteration_count; ++iteration) {
    ...
    while (finished_testcase_count < testcase_list.size()) {
      ...
      for (auto& child_proc : child_proc_list) {
        if (child_proc.pid == 0 && next_testcase_id < static_cast<int>(testcase_list.size())) {
          ...
          if (++next_test_id == static_cast<int>(testcase_list[next_testcase_id].TestCount())) {
            next_test_id = 0;
            ++next_testcase_id;
          }
        }
