...
static fmod_intel_data_t g_fmod_intel_data[] = {
  { // Entry 0
...
TEST(math_fmod, fmod_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_fmod_intel_data)/sizeof(fmod_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_fmod_intel_data[i].expected, fmod(g_fmod_intel_data[i].call_data1, g_fmod_intel_data[i].call_data2)) << "Failed on element " << i;
  }
#else // __BIONIC__
