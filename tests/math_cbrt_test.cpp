...
static cbrt_intel_data_t g_cbrt_intel_data[] = {
  { // Entry 0
    -0x1.p-10,
    -0x1.0p-30
  },
...
TEST(math_cbrt, cbrt_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_cbrt_intel_data)/sizeof(cbrt_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_cbrt_intel_data[i].expected, cbrt(g_cbrt_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
