...
class DlExtRelroShareTest : public DlExtTest,
                            public ::testing::WithParamInterface<bool> {};

TEST_P(DlExtRelroShareTest, ChildWrites) {
  void* start = mmap(NULL, LIBSIZE, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS,
                     -1, 0);
  ASSERT_TRUE(start != MAP_FAILED);
  android_dlextinfo extinfo;
  ...
  int relro_fd;
  if (GetParam()) {
    /* Test also mapping the file after writing */
    ...
    /* Test just writing it without mapping it */
    relro_fd = creat(RELRO_FILE, 0644);
    extinfo.flags = ANDROID_DLEXT_RESERVED_ADDRESS | ANDROID_DLEXT_WRITE_RELRO;
  }
...
INSTANTIATE_TEST_CASE_P(WriteAndUse, DlExtRelroShareTest, ::testing::Bool());
