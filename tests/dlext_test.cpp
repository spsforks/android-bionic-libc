...
TEST(dlext, ns_isolated) {
  ...
  ASSERT_SUBSTR("library \"" + lib_private_external_path + "\" is not accessible for the namespace \"private_isolated1\"", dlerror());
...
TEST(dlext, ns_shared) {
  ...
  void* handle2 = android_dlopen_ext(root_lib, RTLD_NOW, &extinfo);
  ASSERT_TRUE(handle2 == nullptr);
  ASSERT_SUBSTR("library \"" + lib_private_external_path + "\" is not accessible for the namespace \"private_isolated_shared\"", dlerror());
