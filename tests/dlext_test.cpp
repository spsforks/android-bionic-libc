...
TEST(dlext, ns_smoke) {
  ...
  // dlopen for public library using absolute path should work for isolated namespaces
  extinfo.library_namespace = ns2;
...
TEST(dlext, ns_isolated) {
  ...
  ASSERT_TRUE(handle2 == nullptr);
  ASSERT_EQ("dlopen failed: library \"" + lib_private_external_path + "\" in not accessible for the namespace \"private_isolated1\"", dlerror());
  ...
  handle2 = android_dlopen_ext(lib_private_external_path.c_str(), RTLD_NOW, &extinfo);
  ASSERT_TRUE(handle2 == nullptr);
  ASSERT_EQ("dlopen failed: library \"" + lib_private_external_path + "\" in not accessible for the namespace \"private_isolated2\"", dlerror());
