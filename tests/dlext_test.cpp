...
static int lookup_fn(const char* filename, int* fd, off64_t *offset, int* close_flag, void* arg) {
...
TEST_F(DlExtTest, WithLookupFunction) {
  ...
  extinfo.lookup_fn_arg = zip_path;
...
TEST_F(DlExtTest, dlopenWithLookupFn) {
  ...
  typedef void (*Fn)(lookup_fn_t, void*);
  Fn android_update_lookup_fn = reinterpret_cast<Fn>(sym);
  // check if lookup works
  android_update_lookup_fn(lookup_fn, zip_path);
  ...
  dlclose(handle_);
  // reset
  android_update_lookup_fn(nullptr, nullptr);
  handle_ = dlopen("libapk_test_with_dependency.so", RTLD_NOW);
  ASSERT_TRUE(handle_ == NULL);
