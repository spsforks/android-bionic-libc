...
protected:
  virtual void SetUp() {
    ...
    test_lib_dir_ = g_testlib_root + "/" + kTestlibDir;
  }
...
TEST_F(DlExtTest, ExtInfoUseFd) {
  const std::string lib_path = test_lib_dir_ + "/libdlext_test_fd/libdlext_test_fd.so";
...
TEST_F(DlExtTest, ExtInfoUseFdWithOffset) {
  const std::string lib_path = test_lib_dir_ + "/libdlext_test_zip/libdlext_test_zip_zipaligned.zip";
