...
TEST(dlext, ns_link_namespaces_invalid_arguments) {
  ...
  // Test android_link_namespaces_ext()
  ASSERT_FALSE(android_link_namespaces_ext(nullptr, nullptr, "libc.so", false));
  ASSERT_STREQ("android_link_namespaces_ext failed: "
               "error linking namespaces: namespace_from is null.", dlerror());

  ASSERT_FALSE(android_link_namespaces_ext(ns, nullptr, nullptr, false));
  ASSERT_STREQ("android_link_namespaces_ext failed: "
               "error linking namespaces \"private\"->\"(default)\": "
               "the list of shared libraries is empty.", dlerror());

  ASSERT_FALSE(android_link_namespaces_ext(ns, nullptr, "", false));
  ASSERT_STREQ("android_link_namespaces_ext failed: "
               "error linking namespaces \"private\"->\"(default)\": "
               "the list of shared libraries is empty.", dlerror());

  ASSERT_FALSE(android_link_namespaces_ext(ns, nullptr, "libc.so", true));
  ASSERT_STREQ("android_link_namespaces_ext failed: "
               "error linking namespaces \"private\"->\"(default)\": "
               "the list of shared libraries is not empty and allow_all_shared_libs is true.",
               dlerror());
}
...
TEST(dlext, ns_allow_all_shared_libs) {
  ...
  ASSERT_TRUE(android_link_namespaces_ext(ns_a, ns_b, nullptr, true)) << dlerror();
