...
TEST(wchar, limits) { ASSERT_LT(WCHAR_MIN, WCHAR_MAX); }
...
TEST(wchar, wcstod) { ASSERT_DOUBLE_EQ(1.23, wcstod(L"1.23", NULL)); }

TEST(wchar, wcstof) { ASSERT_FLOAT_EQ(1.23f, wcstof(L"1.23", NULL)); }

TEST(wchar, wcstol) { ASSERT_EQ(123L, wcstol(L"123", NULL, 0)); }

TEST(wchar, wcstoll) { ASSERT_EQ(123LL, wcstol(L"123", NULL, 0)); }

TEST(wchar, wcstold) { ASSERT_DOUBLE_EQ(1.23L, wcstold(L"1.23", NULL)); }

TEST(wchar, wcstoul) { ASSERT_EQ(123UL, wcstoul(L"123", NULL, 0)); }

TEST(wchar, wcstoull) { ASSERT_EQ(123ULL, wcstoul(L"123", NULL, 0)); }
...
TEST(wchar, mbsnrtowcs) {
  ...
  memset(dst, 0, sizeof(dst));
  src = s;
  ASSERT_EQ(2U, mbsnrtowcs(dst, &src, 2, 123,
                           NULL));  // glibc chokes on SIZE_MAX here.
  ASSERT_EQ(L'h', dst[0]);
  ASSERT_EQ(L'e', dst[1]);
  ASSERT_EQ(&s[2], src);
...
TEST(wchar, wmemmove_smoke) {
  ...
  EXPECT_EQ(wstr + 5,
            wmemmove(wstr + 5, wstr, NUM_WCHARS(sizeof(const_wstr)) - 6));
