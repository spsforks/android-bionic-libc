...
TEST(wchar, wcwidth_non_spacing_special_cases) {
  ...
  // This doesn't obviusly make much sense unless you assume that you
  // elide it completely (rather than try to render it) if you're _not_
  // expecting it to show up as an actual hyphen, but that's what Markus
  // Kuhn's original wcwidth.c did, and glibc and iOS do the same.
