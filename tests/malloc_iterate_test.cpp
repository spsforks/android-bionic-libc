...
static void VerifyPtrs(TestDataType* test_data) {
  ...
  // for the pointers.
  malloc_disable();
  bool parsed = android::procinfo::ReadMapFileAsyncSafe("/proc/self/maps", callback);
  malloc_enable();
...
TEST(malloc_iterate, invalid_pointers) {
  ...
  // Only attempt to get memory data for maps that are not from the
  // native allocators.
  std::function<void(uint64_t, uint64_t, uint16_t, uint64_t, ino_t, const char*)> callback =
      [&](uint64_t start, uint64_t end, uint16_t, uint64_t, ino_t, const char* name) {
    if (strcmp(name, "[anon:libc_malloc]") != 0) {
      ...
      malloc_iterate(start, end - start, SavePointers, &test_data);
      total = total - test_data.total_allocated_bytes;
      if (total > 0) {
        char buffer[256];
        int len = 0;
        if (name[0] != '\0') {
