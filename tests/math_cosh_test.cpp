...
static cosh_intel_data_t g_cosh_intel_data[] = {
  { // Entry 0
...
TEST(math_cosh, cosh_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_cosh_intel_data)/sizeof(cosh_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_cosh_intel_data[i].expected, cosh(g_cosh_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
