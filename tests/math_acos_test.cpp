...
static acos_intel_data_t g_acos_intel_data[] = {
  { // Entry 0
...
TEST(math_acos, acos_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_acos_intel_data)/sizeof(acos_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_acos_intel_data[i].expected, acos(g_acos_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
