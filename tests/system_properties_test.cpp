...
TEST(properties, __system_property_reload_no_op) {
  ...
  __system_property_read_callback(
      readptr,
      [](void*, const char*, const char* value, unsigned) { ASSERT_STREQ("test_value", value); },
      &expected_name);
#else   // __BIONIC__
  GTEST_SKIP() << "bionic-only test";
#endif  // __BIONIC__
...
TEST(properties, __system_property_reload_invalid) {
  ...
  ASSERT_EQ(-1, access("/dev/__properties__/property_info", R_OK));
  // Create an invalid property_info file, so the system will attempt to initialize a
  // ContextSerialized
  FILE* fptr = fopen("/dev/__properties__/property_info", "w");
  fclose(fptr);
...
TEST(properties, __system_property_reload_valid) {
  ...
  ASSERT_EQ(0, __system_property_find(property_name.c_str()));
  ASSERT_EQ(0, __system_property_set(property_name.c_str(), value));
