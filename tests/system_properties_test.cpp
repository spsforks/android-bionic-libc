...
struct LocalPropertyTestState {
  LocalPropertyTestState() : valid(false) {
    ...
    if (!dirname) {
      fprintf(stderr,
              "making temp file for test state failed (is %s writable?): %s",
...
TEST(properties, fill) {
  ...
  while (true) {
    ...
    ret = __system_property_add(prop_name, PROP_NAME_MAX - 1, prop_value,
                                PROP_VALUE_MAX - 1);
    if (ret < 0) break;
  ...
  for (int i = 0; i < count; i++) {
    ...
    ASSERT_EQ(PROP_VALUE_MAX - 1,
              __system_property_get(prop_name, prop_value_ret));
...
TEST(properties, fill_hierarchical) {
  ...
  for (int i = 0; i < 8; i++) {
    for (int j = 0; j < 8; j++) {
      for (int k = 0; k < 8; k++) {
        ret = snprintf(prop_name, PROP_NAME_MAX - 1, "property_%d.%d.%d", i, j,
                       k);
        memset(prop_name + ret, 'a', PROP_NAME_MAX - 1 - ret);
        ret =
            snprintf(prop_value, PROP_VALUE_MAX - 1, "value_%d.%d.%d", i, j, k);
        memset(prop_value + ret, 'b', PROP_VALUE_MAX - 1 - ret);
        prop_name[PROP_NAME_MAX - 1] = 0;
        prop_value[PROP_VALUE_MAX - 1] = 0;

        ASSERT_EQ(0, __system_property_add(prop_name, PROP_NAME_MAX - 1,
                                           prop_value, PROP_VALUE_MAX - 1));
      }
    }
  }
...
bool KilledByFault::operator()(int exit_status) const {
  return WIFSIGNALED(exit_status) &&
         (WTERMSIG(exit_status) == SIGSEGV || WTERMSIG(exit_status) == SIGBUS ||
...
TEST_F(properties_DeathTest, read_only) {
  ...
  // This test only makes sense if we're talking to the real system property
  // service.
  struct stat sb;
