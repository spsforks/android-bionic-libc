...
struct LocalPropertyTestState {
  LocalPropertyTestState() : valid(false) {
    ...
    if (!dirname) {
      fprintf(stderr, "making temp file for test state failed (is %s writable?): %s", dir_template,
              strerror(errno));
      return;
    }
  ...
  for (int i = 0; i < 8; i++) {
    for (int j = 0; j < 8; j++) {
      for (int k = 0; k < 8; k++) {
        ...
        ASSERT_EQ(
            0, __system_property_add(prop_name, PROP_NAME_MAX - 1, prop_value, PROP_VALUE_MAX - 1));
      }
    }
  }
