...
static rint_intel_data_t g_rint_intel_data[] = {
  { // Entry 0
    -0.0,
    -0x1.0p-1074
  },
...
TEST(math_rint, rint_intel) {
#if defined(__BIONIC__)
  fesetround(FE_TONEAREST);
  for (size_t i = 0; i < sizeof(g_rint_intel_data)/sizeof(rint_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_rint_intel_data[i].expected, rint(g_rint_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
