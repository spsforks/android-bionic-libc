...
static void* ConcurrentPtsNameFn(void* other_ptsname) {
  int fd = getpt();
  bool differ = (strcmp(reinterpret_cast<const char*>(other_ptsname), ptsname(fd)) != 0);
  close(fd);
  return reinterpret_cast<void*>(differ);
}
#endif // __BIONIC__

// glibc's ptsname isn't thread safe.
TEST(stdlib, ptsname_concurrent) {
#if defined(__BIONIC__)
  int fd = getpt();
  char* name1 = ptsname(fd);
  char* name1_copy = strdup(name1);

  pthread_t t;
  ASSERT_EQ(0, pthread_create(&t, NULL, ConcurrentPtsNameFn, name1));
  void* result;
  ASSERT_EQ(0, pthread_join(t, &result));
  ASSERT_TRUE(static_cast<bool>(result));

  ASSERT_STREQ(name1_copy, name1);
  free(name1_copy);
#else // __BIONIC__
...
TEST(stdlib, ptsname_ENOTTY) {
  errno = 0;
  ASSERT_EQ(nullptr, ptsname(STDOUT_FILENO));
...
TEST(stdlib, ttyname_ttyname_r) {
  int fd = getpt();
  ASSERT_NE(-1, fd);

  // ttyname and ttyname_r should agree.
...
TEST(stdlib, ttyname_ENOTTY) {
  int fd = open("/dev/null", O_WRONLY);
  errno = 0;
  ASSERT_EQ(nullptr, ttyname(fd));
  ASSERT_EQ(ENOTTY, errno);
  close(fd);
}
