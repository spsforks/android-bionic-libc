...
class MembarrierBarrierTest : public ::testing::TestWithParam<::testing::tuple<int,int>> {};

TEST_P(MembarrierBarrierTest, Commands) {
  if (!HasMembarrier()) {
    return;
  }
  ...
  const int membarrier_cmd_register = ::testing::get<0>(GetParam());
  const int membarrier_cmd_barrier = ::testing::get<1>(GetParam());
...
INSTANTIATE_TEST_CASE_P(membarrier,
                        MembarrierBarrierTest,
                        ::testing::Values(
                             ::testing::make_tuple(
