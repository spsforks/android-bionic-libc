...
template<typename T>
void __attribute__((noreturn)) fork_child(unsigned cpu, T &data) {
...
std::string show_errno() { return std::string("Errno was: ") + strerror(errno); }

bool are_watchpoints_supported(pid_t child) {
#if defined(__arm__)
  long capabilities;
  ...
  return true;
#else
  // Mips gets a free pass for now.
  (void) child;
  return false;
#endif
}

void set_watchpoint(pid_t child, const void *address, size_t size) {
...
#ifdef __arm__
  ASSERT_EQ(0, ptrace(PTRACE_SETHBPREGS, child, -1, &address)) << show_errno();
  ASSERT_EQ(0, ptrace(PTRACE_SETHBPREGS, child, -2, &control)) << show_errno();
#else // aarch64
  user_hwdebug_state dreg_state;
  ...
  ASSERT_EQ(0, ptrace(PTRACE_SETREGSET, child, NT_ARM_HW_WATCH, &iov)) << show_errno();
#endif
#elif defined(__i386__) || defined(__x86_64__)
  ASSERT_EQ(0, ptrace(PTRACE_POKEUSER, child, offsetof(user, u_debugreg[0]), address)) << show_errno();
  errno = 0;
  ...
  data &= mask;
  data |= value;
  ASSERT_EQ(0, ptrace(PTRACE_POKEUSER, child, offsetof(user, u_debugreg[7]), data)) << show_errno();
#else
  (void) child;
  (void) address;
  (void) size;
...
template<typename T>
void run_watchpoint_test_impl(unsigned cpu) {
  alignas(8) T data = 0;

  pid_t child = fork();
  ASSERT_NE(-1, child) << show_errno();
  if (child == 0)
    fork_child(cpu, data);
  ...
  int status;
  ASSERT_EQ(child, waitpid(child, &status, __WALL)) << show_errno();
  ...
  ASSERT_EQ(0, ptrace(PTRACE_CONT, child, nullptr, nullptr)) << show_errno();
  ASSERT_EQ(child, waitpid(child, &status, __WALL)) << show_errno();
  ASSERT_TRUE(WIFSTOPPED(status)) << "Status was: " << status;
  ASSERT_EQ(SIGTRAP, WSTOPSIG(status)) << "Status was: " << status;

  siginfo_t siginfo;
  ASSERT_EQ(0, ptrace(PTRACE_GETSIGINFO, child, nullptr, &siginfo)) << show_errno();
...
void run_watchpoint_test(unsigned cpu) {
