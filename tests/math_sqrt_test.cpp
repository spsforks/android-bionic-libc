...
static sqrt_intel_data_t g_sqrt_intel_data[] = {
  { // Entry 0
...
TEST(math_sqrt, sqrt_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_sqrt_intel_data)/sizeof(sqrt_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_sqrt_intel_data[i].expected, sqrt(g_sqrt_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
