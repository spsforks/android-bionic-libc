...
static log1p_intel_data_t g_log1p_intel_data[] = {
  { // Entry 0
...
TEST(math_log1p, log1p_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_log1p_intel_data)/sizeof(log1p_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_log1p_intel_data[i].expected, log1p(g_log1p_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
