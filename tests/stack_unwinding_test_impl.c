    ...
    )
{
    if (action & _UA_END_OF_STACK) {
        // we reach end of stack and don't execute foo1_handler. test failed.
        printf("abort...\n");
        exit(0);
    }
...
static void __attribute__((noinline)) foo2() {
    char* ptr = NULL; // this null pointer will be dereferenced below
    *ptr = 0xFF; // raise SEGFAULT here !
}
