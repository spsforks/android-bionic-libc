...
struct StringTestState {
  ...
  bool NextIteration() {
    if (align1_index_ == sizeof(alignments) && align2_index_ == sizeof(alignments)) {
      return false;
    }

    if (align1_index_ == sizeof(alignments)) {
      align1_index_ = 0;
      align2_index_++;
    } else {
      align1_index_++;
    ...
    ptr = glob_ptr + align1_index_;
    ptr1 = glob_ptr1 + align1_index_;
    ptr2 = glob_ptr2 + align2_index_;
...
template<class Character>
size_t StringTestState<Character>::alignments[] = { 0, 1, 2, 3, 4, 5, 6, 7, 11 };

TEST(string, strcat) {
  StringTestState<char> state(SMALL);
  for (size_t i = 1; i < state.n; i++) {
    for (state.BeginIterations(); state.NextIteration(); ) {
