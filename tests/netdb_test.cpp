...
TEST(netdb, gethostbyname_invalid_host) {
  h_errno = 0;
  ASSERT_EQ(NULL, gethostbyname("invalid_host"));
  ASSERT_EQ(HOST_NOT_FOUND, h_errno);
}

TEST(netdb, gethostbyname2_invalid_host) {
  h_errno = 0;
  ASSERT_EQ(NULL, gethostbyname2("invalid_host", AF_INET));
  ASSERT_EQ(HOST_NOT_FOUND, h_errno);
  h_errno = 0;
  ASSERT_EQ(NULL, gethostbyname2("invalid_host", AF_INET6));
  ASSERT_EQ(HOST_NOT_FOUND, h_errno);
}

TEST(netdb, gethostbyname_r_invalid_host) {
  hostent hent;
  hostent *hp;
  char buf[512];
  int err = 0;
  gethostbyname_r("invalid_host", &hent, buf, sizeof(buf), &hp, &err);
  ASSERT_EQ(NULL, hp);
  ASSERT_EQ(HOST_NOT_FOUND, err);
  // return result and h_errno not checked, as glibc will also fail on the check.
}

TEST(netdb, gethostbyname2_r_invalid_host) {
  hostent hent;
  hostent *hp;
  char buf[512];
  int err = 0;
  gethostbyname2_r("invalid_host", AF_INET, &hent, buf, sizeof(buf), &hp, &err);
  ASSERT_EQ(NULL, hp);
  ASSERT_EQ(HOST_NOT_FOUND, err);
  // return result and h_errno not checked, as glibc will also fail on the check.
}
