...
static acoshf_intel_data_t g_acoshf_intel_data[] = {
  { // Entry 0
...
TEST(math_acoshf, acoshf_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_acoshf_intel_data)/sizeof(acoshf_intel_data_t); i++) {
    EXPECT_FLOAT_EQ(g_acoshf_intel_data[i].expected, acoshf(g_acoshf_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
