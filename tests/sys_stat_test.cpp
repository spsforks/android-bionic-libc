...
TEST(sys_stat, faccessat_EINVAL) {
  ...
  ASSERT_EQ(EINVAL, errno);
  ASSERT_EQ(-1, faccessat(AT_FDCWD, "/dev/null", F_OK, ~AT_EACCESS));
  ASSERT_EQ(EINVAL, errno);
#if defined(__BIONIC__)
  ASSERT_EQ(-1, faccessat(AT_FDCWD, "/dev/null", ~(R_OK | W_OK | X_OK), 0));
  ASSERT_EQ(EINVAL, errno);
#else
  // glibc 2.19 allows any mode ...
  ASSERT_EQ(0, faccessat(AT_FDCWD, "/dev/null", ~(R_OK | W_OK | X_OK), AT_SYMLINK_NOFOLLOW));
  // ... except if flags is 0, at which point EINVAL is returned. WTF?
  ASSERT_EQ(-1, faccessat(AT_FDCWD, "/dev/null", ~(R_OK | W_OK | X_OK), 0));
  ASSERT_EQ(EINVAL, errno);
#endif
}
...
TEST(sys_stat, faccessat_dev_null) {
  ASSERT_EQ(0, faccessat(AT_FDCWD, "/dev/null", F_OK, AT_EACCESS));
  ASSERT_EQ(0, faccessat(AT_FDCWD, "/dev/null", R_OK, AT_EACCESS));
  ASSERT_EQ(0, faccessat(AT_FDCWD, "/dev/null", W_OK, AT_EACCESS));
  ASSERT_EQ(0, faccessat(AT_FDCWD, "/dev/null", R_OK|W_OK, AT_EACCESS));
}
