...
static void CheckFileMode(int fd, mode_t expected_mode) {
  struct stat sb;
  ASSERT_EQ(0, fstat(fd, &sb));
...
static void CheckSymlinkMode(const char* linkname, mode_t expected_mode) {
  struct stat sb;
...
TEST(sys_stat, fchmodat_AT_SYMLINK_NOFOLLOW_with_symlink) {
  ...
  ASSERT_EQ(0, fstat(tf.fd, &target_sb));
  ...
  if (result == 0) {
    CheckSymlinkMode(linkname, 0751);
  } else {
    ASSERT_EQ(-1, result);
    ASSERT_EQ(ENOTSUP, errno);
  }
  ...
  CheckFileMode(tf.fd, target_sb.st_mode);
