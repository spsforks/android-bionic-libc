...
static round_intel_data_t g_round_intel_data[] = {
  { // Entry 0
    0x1.p0,
    0x1.0p-1
  },
...
TEST(math_round, round_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_round_intel_data)/sizeof(round_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_round_intel_data[i].expected, round(g_round_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
