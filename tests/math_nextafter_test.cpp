...
typedef struct {
  ...
  double call_data1;
  double call_data2;
} nextafter_intel_data_t;

static nextafter_intel_data_t g_nextafter_intel_data[] = {
  { // Entry 0
    -0x1.p-1074,
    -0x1.0p-1074,
    -0x1.0p-1074
...
TEST(math_nextafter, nextafter_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_nextafter_intel_data)/sizeof(nextafter_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_nextafter_intel_data[i].expected, nextafter(g_nextafter_intel_data[i].call_data1, g_nextafter_intel_data[i].call_data2)) << "Failed on element " << i;
  }
#else // __BIONIC__
