...
struct FpUlpEq {
  ::testing::AssertionResult operator()(const char* /* expected_expression */,
                                        const char* /* actual_expression */, T expected, T actual) {
    ...
    return ::testing::AssertionFailure() << "expected (" << expected_str << ") != actual ("
                                         << actual_str << ")";
  }
};
...
void DoMathDataTest(data_1_1_t<RT, T>(&data)[N], RT f(T)) {
  ...
  for (size_t i = 0; i < N; ++i) {
    EXPECT_PRED_FORMAT2(predicate, data[i].expected, f(data[i].input)) << "Failed on element " << i;
  }
}
...
void DoMathDataTest(data_1_2_t<RT, T1, T2>(&data)[N], RT f(T1, T2)) {
  ...
  FpUlpEq<ULP, RT> predicate;
  for (size_t i = 0; i < N; ++i) {
    EXPECT_PRED_FORMAT2(predicate, data[i].expected, f(data[i].input1, data[i].input2))
        << "Failed on element " << i;
  }
}
