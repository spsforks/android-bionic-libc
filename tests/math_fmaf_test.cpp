...
static fmaf_intel_data_t g_fmaf_intel_data[] = {
  { // Entry 0
...
TEST(math_fmaf, fmaf_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_fmaf_intel_data)/sizeof(fmaf_intel_data_t); i++) {
    EXPECT_FLOAT_EQ(g_fmaf_intel_data[i].expected, fmaf(g_fmaf_intel_data[i].call_data1, g_fmaf_intel_data[i].call_data2, g_fmaf_intel_data[i].call_data3)) << "Failed on element " << i;
  }
#else // __BIONIC__
