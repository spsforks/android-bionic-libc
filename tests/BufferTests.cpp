...
static int g_single_aligns[][2] = {
  // When alignments are set to zero, the default return value of malloc is
  // used.
  { 0, 0 },
...
void RunSingleBufferAlignTest(
  ...
  // guarantee that the all possible alignments are will fit.
  uint8_t *buf = reinterpret_cast<uint8_t*>(malloc(3*max_test_size));
...
void RunSrcDstBufferAlignTest(
  ...
  uint8_t* src = reinterpret_cast<uint8_t*>(malloc(3*max_test_size));
  ASSERT_TRUE(src != NULL);
  uint8_t* dst = reinterpret_cast<uint8_t*>(malloc(3*max_test_size));
...
void RunSingleBufferOverreadTest(void (*test_func)(uint8_t*, size_t)) {
  ...
  uint8_t* memory = reinterpret_cast<uint8_t*>(memalign(pagesize, 2 * pagesize));
...
void RunSrcDstBufferOverreadTest(void (*test_func)(uint8_t*, uint8_t*, size_t)) {
  ...
  uint8_t* dst = reinterpret_cast<uint8_t*>(malloc(pagesize));
  ASSERT_TRUE(dst != NULL);
  for (size_t i = 0; i < pagesize; i++) {
