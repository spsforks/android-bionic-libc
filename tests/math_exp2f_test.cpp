...
static exp2f_intel_data_t g_exp2f_intel_data[] = {
  { // Entry 0
...
TEST(math_exp2f, exp2f_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_exp2f_intel_data)/sizeof(exp2f_intel_data_t); i++) {
    EXPECT_FLOAT_EQ(g_exp2f_intel_data[i].expected, exp2f(g_exp2f_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
