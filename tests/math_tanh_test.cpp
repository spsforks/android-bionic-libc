...
static tanh_intel_data_t g_tanh_intel_data[] = {
  { // Entry 0
...
TEST(math_tanh, tanh_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_tanh_intel_data)/sizeof(tanh_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_tanh_intel_data[i].expected, tanh(g_tanh_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
