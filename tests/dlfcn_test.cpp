...
TEST(dlfcn, dlopen_simple) {
  void* handle = dlopen(LIB_PATH "/tglb/libtestGoldbach.so", RTLD_NOW);
  ASSERT_TRUE(handle != NULL);
  ASSERT_EQ(0, dlclose(handle));
}

TEST(dlfcn, dlopen_library_with_runpath) {
  void* handle = dlopen(LIB_PATH "/tcltz/libtestCollatz.so", RTLD_NOW);
  ASSERT_TRUE(handle != NULL);
  ASSERT_EQ(0, dlclose(handle));
}
...
TEST(dlfcn, dlopen_one_library_twice_using_different_path) {
  void* handle = dlopen(LIB_PATH "/tglb/libtestGoldbach.so", RTLD_NOW);
  ASSERT_TRUE(handle != NULL);
  void* handle2 = dlopen(LIB_PATH "/tcltz/../tglb/libtestGoldbach.so", RTLD_NOW);
