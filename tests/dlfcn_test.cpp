...
TEST(dlfcn, dlopen_check_order_reloc_siblings) {
  ...
  // +-> ..._1.so <- empty
  // |   |
  // |   +-> ..._a.so <- exports right answer_impl()
  // |   |
  // |   +-> ..._b.so <- every other letter exporting wrong one.
  // |
  // +-> ..._2.so <- empty
  // |   |
  // |   +-> ..._c.so
...
TEST(dlfcn, dlopen_check_order_reloc_nephew) {
  ...
  // |   +-> ..._d.so
  // |
  // +-> ..._3.so <- get_answer2() that calls get_anser_impl();
  //     |
  //     +-> ..._e.so
  //     |
  //     +-> ..._f.so <- exports get_answer() that calls get_anser_impl();
  ...
  typedef int (*fn_t) (void);
  fn_t fn = reinterpret_cast<fn_t>(dlsym(handle, "check_order_reloc_siblings_get_answer2"));
  ASSERT_TRUE(fn != nullptr) << dlerror();
  ASSERT_EQ(42, fn());
