...
// libtest_dlopen_from_ctor_main.so depends on the
// libtest_dlopen_from_ctor.so which has constructor
// that calls dlopen(libc...). This is to test situation
...
TEST(dlfcn, dlopen_dlopen_from_ctor) {
  ...
  dlclose(handle);
#else
  GTEST_LOG_(INFO) << "This test is disabled for glibc (glibc segfaults if you try to call dlopen from a contructor).\n";
#endif
}
