...
static log2_intel_data_t g_log2_intel_data[] = {
  { // Entry 0
...
TEST(math_log2, log2_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_log2_intel_data)/sizeof(log2_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_log2_intel_data[i].expected, log2(g_log2_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
