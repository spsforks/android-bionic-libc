...
static expm1_intel_data_t g_expm1_intel_data[] = {
  { // Entry 0
...
TEST(math_expm1, expm1_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_expm1_intel_data)/sizeof(expm1_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_expm1_intel_data[i].expected, expm1(g_expm1_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
