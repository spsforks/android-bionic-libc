...
static exp2_intel_data_t g_exp2_intel_data[] = {
  { // Entry 0
...
TEST(math_exp2, exp2_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_exp2_intel_data)/sizeof(exp2_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_exp2_intel_data[i].expected, exp2(g_exp2_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
