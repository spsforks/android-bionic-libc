...
static ldexp_intel_data_t g_ldexp_intel_data[] = {
  { // Entry 0
    -0.0,
    -0x1.0p-1074,
    (int)-10
...
TEST(math_ldexp, ldexp_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_ldexp_intel_data)/sizeof(ldexp_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_ldexp_intel_data[i].expected, ldexp(g_ldexp_intel_data[i].call_data1, g_ldexp_intel_data[i].call_data2)) << "Failed on element " << i;
  }
#else // __BIONIC__
