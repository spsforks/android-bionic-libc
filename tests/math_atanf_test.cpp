...
static atanf_intel_data_t g_atanf_intel_data[] = {
  { // Entry 0
...
TEST(math_atanf, atanf_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_atanf_intel_data)/sizeof(atanf_intel_data_t); i++) {
    EXPECT_FLOAT_EQ(g_atanf_intel_data[i].expected, atanf(g_atanf_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
