...
static fabs_intel_data_t g_fabs_intel_data[] = {
  { // Entry 0
    0x1.p-10,
    -0x1.0p-10
  },
...
TEST(math_fabs, fabs_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_fabs_intel_data)/sizeof(fabs_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_fabs_intel_data[i].expected, fabs(g_fabs_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
