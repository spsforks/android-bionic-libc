...
INSTANTIATE_TEST_SUITE_P(, MemtagStackTest,
                         testing::Combine(testing::Values("vfork_exit", "vfork_exec", "longjmp",
                                                          "longjmp_sigaltstack"),
                                          testing::Bool()),
                         [](const ::testing::TestParamInfo<MemtagStackTest::ParamType>& info) {
                           std::string s = get<0>(info.param);
                           if (get<1>(info.param)) s += "_static";
                           return s;
                         });
