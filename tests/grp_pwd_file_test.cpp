...
void ExpectEntryDoesntExist(File* file, const char* name, Id id) {
  typename File::Fields line;
  EXPECT_FALSE(FindByName(file, name, &line)) << name;
  EXPECT_FALSE(FindById(file, id, &line)) << id;
}
...
TEST(grp_pwd_file, passwd_file_one_entry) {
  ...
  ExpectEntryDoesntExist(&passwd_file, "not_name", 2);
...
TEST(grp_pwd_file, group_file_one_entry) {
  ...
  ExpectEntryDoesntExist(&group_file, "not_name", 2);
...
TEST(grp_pwd_file, passwd_file_many_entries) {
  ...
  ExpectEntryDoesntExist(&passwd_file, "not_name", 50);
...
TEST(grp_pwd_file, group_file_many_entries) {
  ...
  ExpectEntryDoesntExist(&group_file, "not_name", 799);
