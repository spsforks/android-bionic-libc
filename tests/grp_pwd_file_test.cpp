...
void FindAndCheckPasswdEntry(PasswdFile* file, const char* name, uid_t uid, gid_t gid,
  ...
  PasswdFields name_fields;
  ASSERT_TRUE(FindByName(file, name, &name_fields));
  passwd name_passwd;
  ASSERT_TRUE(name_fields.ToPasswd(&name_passwd));
  ...
  PasswdFields id_fields;
  ASSERT_TRUE(FindById(file, 1, &id_fields));

  passwd id_passwd;
  ASSERT_TRUE(name_fields.ToPasswd(&id_passwd));
...
void FindAndCheckGroupEntry(GroupFile* file, const char* name, gid_t gid, const char** members) {
  GroupFields name_fields;
  ASSERT_TRUE(FindByName(file, name, &name_fields)) << name;
  group name_group;
  ASSERT_TRUE(name_fields.ToGroup(&name_group));

  EXPECT_STREQ(name, name_group.gr_name);
  EXPECT_EQ(nullptr, name_group.gr_passwd);
  EXPECT_EQ(gid, name_group.gr_gid);
  EXPECT_EQ(members, name_group.gr_mem);

  GroupFields id_fields;
  ASSERT_TRUE(FindById(file, 1, &id_fields));

  group id_group;
  ASSERT_TRUE(name_fields.ToGroup(&id_group));
...
TEST(grp_pwd_file, passwd_file_one_entry) {
  ...
  write(file.fd, test_string, sizeof(test_string));
...
TEST(grp_pwd_file, group_file_one_entry) {
  ...
  FindAndCheckGroupEntry(&group_file, "name", 1, nullptr);
...
TEST(grp_pwd_file, passwd_file_many_entries) {
  ...
  static const char test_string[] =
      ...
      "middle-ish::13:4::/:middle-ish-shell-parameter-is-longer-than-32-char\n"
      "abc7::15:4:abc::\n"
      "abc8::17:4:::\n"
  ...
  FindAndCheckPasswdEntry(&passwd_file, "middle-ish", 13, 4, "/",
                          "middle-ish-shell-parameter-is-longer-than-32-char");
...
TEST(grp_pwd_file, group_file_many_entries) {
  ...
  FindAndCheckGroupEntry(&group_file, "first", 1, nullptr);
  FindAndCheckGroupEntry(&group_file, "middle-ish", 6, nullptr);
  FindAndCheckGroupEntry(&group_file, "last", 800, nullptr);
