...
class FileUnmapper {
 public:
  FileUnmapper(MmapFile& file) : file_(file) {
  }
  ~FileUnmapper() {
    const char* start;
    const char* end;
    ...
    munmap(const_cast<char*>(start), size);
  }
...
void FindAndCheckPasswdEntry(MmapFile* file, const char* name, uid_t uid, gid_t gid,
...
void FindAndCheckGroupEntry(MmapFile* file, const char* name, gid_t gid) {
...
TEST(grp_pwd_file, passwd_file_one_entry) {
  ...
  MmapFile passwd_file(file.filename);
  FileUnmapper unmapper(passwd_file);

  FindAndCheckPasswdEntry(&passwd_file, "name", 1, 2, "dir", "shell");

  PasswdLine non_exist_line;
  EXPECT_FALSE(FindByName(&passwd_file, "not_name", &non_exist_line));
  EXPECT_FALSE(FindById(&passwd_file, 3, &non_exist_line));
...
TEST(grp_pwd_file, group_file_one_entry) {
  ...
  MmapFile group_file(file.filename);
  FileUnmapper unmapper(group_file);

  FindAndCheckGroupEntry(&group_file, "name", 1);

  GroupLine non_exist_line;
  EXPECT_FALSE(FindByName(&group_file, "not_name", &non_exist_line));
  EXPECT_FALSE(FindById(&group_file, 3, &non_exist_line));
...
TEST(grp_pwd_file, passwd_file_many_entries) {
  ...
  PasswdLine non_exist_line;
  EXPECT_FALSE(FindByName(&passwd_file, "not_name", &non_exist_line));
  EXPECT_FALSE(FindById(&passwd_file, 50, &non_exist_line));
...
TEST(grp_pwd_file, group_file_many_entries) {
  ...
  GroupLine non_exist_line;
  EXPECT_FALSE(FindByName(&group_file, "not_name", &non_exist_line));
  EXPECT_FALSE(FindById(&group_file, 799, &non_exist_line));
