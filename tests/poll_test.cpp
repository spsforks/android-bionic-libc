...
TEST(poll, poll)
{
    ...
    ASSERT_EQ(0, pthread_create (&threadWrite, NULL, threadWriteBody, NULL));
    ASSERT_EQ(0, pthread_create (&threadPoll, NULL, threadPollBody, NULL));

    EXPECT_EQ(0, pthread_join (threadWrite, &retWrite));
    EXPECT_EQ(0, pthread_join (threadPoll, &retPoll));

    ASSERT_EQ(NULL, retWrite);
    ASSERT_EQ(NULL, retPoll);
}
