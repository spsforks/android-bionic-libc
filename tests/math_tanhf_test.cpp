...
static tanhf_intel_data_t g_tanhf_intel_data[] = {
  { // Entry 0
...
TEST(math_tanhf, tanhf_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_tanhf_intel_data)/sizeof(tanhf_intel_data_t); i++) {
    EXPECT_FLOAT_EQ(g_tanhf_intel_data[i].expected, tanhf(g_tanhf_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
