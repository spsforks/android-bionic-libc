...
static trunc_intel_data_t g_trunc_intel_data[] = {
  { // Entry 0
    -0.0,
    -0x1.0p-1074
  },
...
TEST(math_trunc, trunc_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_trunc_intel_data)/sizeof(trunc_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_trunc_intel_data[i].expected, trunc(g_trunc_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
