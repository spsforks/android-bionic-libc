...
static asinhf_intel_data_t g_asinhf_intel_data[] = {
  { // Entry 0
...
TEST(math_asinhf, asinhf_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_asinhf_intel_data)/sizeof(asinhf_intel_data_t); i++) {
    EXPECT_FLOAT_EQ(g_asinhf_intel_data[i].expected, asinhf(g_asinhf_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
