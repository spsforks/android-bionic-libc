...
void test_write_size() {
  char buf[4] = {0};
  // NOLINTNEXTLINE(whitespace/line_length)
  // GCC: error: call to '__write_too_big_error' declared with attribute error: write called with size bigger than destination
  // clang should emit a warning, but doesn't
  write(STDOUT_FILENO, buf, 5);
}

void test_write_too_big() {
  void *buf = calloc(atoi("5"), 1);
  // NOLINTNEXTLINE(whitespace/line_length)
  // GCC: error: call to '__write_count_toobig_error' declared with attribute error: write called with count > SSIZE_MAX
  // clang should emit a warning, but doesn't
  write(STDOUT_FILENO, buf, SIZE_MAX);
}
