...
static ilogb_intel_data_t g_ilogb_intel_data[] = {
  { // Entry 0
    (int)0x1.90p6,
    0x1.0p100
  },
...
TEST(math_ilogb, ilogb_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_ilogb_intel_data)/sizeof(ilogb_intel_data_t); i++) {
    EXPECT_EQ(g_ilogb_intel_data[i].expected, ilogb(g_ilogb_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
