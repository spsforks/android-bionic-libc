...
TEST_F(pthread_DeathTest, pthread_setname_np__no_such_thread) {
  ...
  ASSERT_EXIT(pthread_setname_np(dead_thread, "short 3"), testing::KilledBySignal(SIGABRT),
              "No thread found for pthread id");
#else
...
#if defined(__BIONIC__)
  ASSERT_EXIT(pthread_detach(dead_thread), testing::KilledBySignal(SIGABRT),
              "No thread found for pthread id");
#else
...
#if defined(__BIONIC__)
  ASSERT_EXIT(pthread_join(dead_thread, NULL), testing::KilledBySignal(SIGABRT),
              "No thread found for pthread id");
#else
...
#if defined(__BIONIC__)
  ASSERT_EXIT(pthread_kill(dead_thread, 0), testing::KilledBySignal(SIGABRT),
              "No thread found for pthread id");
#else
