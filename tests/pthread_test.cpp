...
static void* IdFn(void* arg) { return arg; }
...
static void* JoinFn(void* arg) {
  return reinterpret_cast<void*>(
      pthread_join(reinterpret_cast<pthread_t>(arg), NULL));
}
...
TEST(pthread, pthread_create_EAGAIN) {
  ...
  ASSERT_EQ(0,
            pthread_attr_setstacksize(
                &attributes, static_cast<size_t>(-1) & ~(getpagesize() - 1)));
...
struct TestBug37410 {
  ...
  static void main() {
    ...
    pthread_t t;
    ASSERT_EQ(0, pthread_create(&t, NULL, TestBug37410::thread_fn,
...
static void* SignalHandlerFn(void* arg) {
  ...
  sigfillset(&wait_set);
  return reinterpret_cast<void*>(
      sigwait(&wait_set, reinterpret_cast<int*>(arg)));
}
...
TEST(pthread, pthread_sigmask) {
  ...
  ASSERT_EQ(0, pthread_create(&signal_thread, NULL, SignalHandlerFn,
                              &received_signal));
...
TEST(pthread, pthread_setname_np__too_long) {
  ASSERT_EQ(ERANGE, pthread_setname_np(pthread_self(),
                                       "this name is far too long for linux"));
}
...
static void OnceFn() { ++g_once_fn_call_count; }
...
static void Routine2() { pthread_once_1934122_result += "2"; }
...
TEST(pthread, pthread_attr_getstack__main_thread) {
  ...
  while (fgets(line, sizeof(line), fp) != NULL) {
    uintptr_t lo, hi;
    char name[10];
    sscanf(line, "%" PRIxPTR "-%" PRIxPTR " %*4s %*x %*x:%*x %*d %10s", &lo,
           &hi, name);
  ...
  rl.rlim_cur = rl.rlim_max =
      1024;  // 1KiB. We know the stack must be at least a page already.
...
static void AbortCleanupRoutine(void*) { abort(); }

static void CountCleanupRoutine(void*) { ++cleanup_counter; }
