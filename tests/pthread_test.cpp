  ...
  EXPECT_EXIT(
      {
        ...
        if (pthread_mutexattr_init(&muattr) == -1) {
          exit(1);
        }
        if (pthread_mutex_init(&sync.mutex, &muattr) == -1) {
          exit(1);
        }
        ...
        pthread_create(
            &id, &attr,
            [](void* d) -> void* {
              Sync* sync = reinterpret_cast<Sync*>(d);
              pthread_mutex_lock(&sync->mutex);
              sync->state = kThreadStarted;
              pthread_mutex_unlock(&sync->mutex);
              pthread_cond_signal(&sync->cond);
              while (sync->state != kStackRemapped) {
        ...
        pthread_mutex_lock(&sync.mutex);
        while (sync.state != kThreadStarted) {
          pthread_cond_wait(&sync.cond, &sync.mutex);
        }
        __pthread_internal_remap_stack_with_mte();
        sync.state = kStackRemapped;
        pthread_mutex_unlock(&sync.mutex);
        pthread_cond_signal(&sync.cond);
