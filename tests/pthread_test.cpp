...
struct PthreadRwlockWakeupHelperArg {
  ...
  volatile enum {
    PROGRESS_IN_LOCK_INIT,
    PROGRESS_IN_LOCK_WAITING,
    PROGRESS_IN_LOCK_RELEASED
  } progress;
};

static void pthread_rwlock_reader_wakeup_writer_helper(PthreadRwlockWakeupHelperArg* arg) {
  arg->progress = PthreadRwlockWakeupHelperArg::PROGRESS_IN_LOCK_WAITING;
  ASSERT_EQ(EBUSY, pthread_rwlock_trywrlock(&arg->lock));
  ASSERT_EQ(0, pthread_rwlock_wrlock(&arg->lock));
  ASSERT_EQ(PthreadRwlockWakeupHelperArg::PROGRESS_IN_LOCK_RELEASED, arg->progress);
...
TEST(pthread, pthread_rwlock_reader_wakeup_writer) {
  ...
  wakeup_arg.progress = PthreadRwlockWakeupHelperArg::PROGRESS_IN_LOCK_INIT;

  pthread_t thread;
  ASSERT_EQ(0, pthread_create(&thread, NULL,
    reinterpret_cast<void* (*)(void*)>(pthread_rwlock_reader_wakeup_writer_helper), &wakeup_arg));
  sleep(1);
  ASSERT_EQ(PthreadRwlockWakeupHelperArg::PROGRESS_IN_LOCK_WAITING, wakeup_arg.progress);
  wakeup_arg.progress = PthreadRwlockWakeupHelperArg::PROGRESS_IN_LOCK_RELEASED;
