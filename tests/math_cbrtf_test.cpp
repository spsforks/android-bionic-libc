...
static cbrtf_intel_data_t g_cbrtf_intel_data[] = {
  { // Entry 0
...
TEST(math_cbrtf, cbrtf_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_cbrtf_intel_data)/sizeof(cbrtf_intel_data_t); i++) {
    EXPECT_FLOAT_EQ(g_cbrtf_intel_data[i].expected, cbrtf(g_cbrtf_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
