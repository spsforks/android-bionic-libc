...
static hypot_intel_data_t g_hypot_intel_data[] = {
  { // Entry 0
...
TEST(math_hypot, hypot_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_hypot_intel_data)/sizeof(hypot_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_hypot_intel_data[i].expected, hypot(g_hypot_intel_data[i].call_data1, g_hypot_intel_data[i].call_data2)) << "Failed on element " << i;
  }
#else // __BIONIC__
