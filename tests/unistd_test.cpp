...
TEST(UNISTD_TEST, sysconf_SC_ARG_MAX) {
  ...
  // expansion of 132KiB (see setup_arg_pages() in fs/exec.c).
  rl.rlim_cur = 1024;
  rl.rlim_max = RLIM_INFINITY;
  ASSERT_EQ(0, setrlimit(RLIMIT_STACK, &rl));

  constexpr long kernel_stack_expand = 131072;
  EXPECT_EQ(kernel_stack_expand, sysconf(_SC_ARG_MAX));

  // With the known kernel stack expand, we know exactly what _SC_ARG_MAX should be...
  rl.rlim_cur = kernel_stack_expand;
  rl.rlim_max = RLIM_INFINITY;
  ASSERT_EQ(0, setrlimit(RLIMIT_STACK, &rl));
