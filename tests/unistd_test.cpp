...
TEST(unistd, _POSIX_macros_smoke) {
...
#if defined(__BIONIC__)
  // Tests can only run on bionic, as bionic and glibc has different support on these macros.
  // Some macros like _POSIX_ADVISORY_INFO are not supported on bionic yet.
  // Some macros like _POSIX_SS_REPL_MAX and _POSIX2_FORT_DEV are not defined in glibc.
  // Macros _POSIX_V7_* are just not exposed by glibc.
  ...
  EXPECT_GT(_POSIX_SS_REPL_MAX, 0);
  EXPECT_EQ(-1, _POSIX_THREAD_PROCESS_SHARED);
  EXPECT_EQ(-1, _POSIX_THREAD_ROBUST_PRIO_INHERIT);
  EXPECT_GT(_POSIX_TRACE_EVENT_NAME_MAX, 0);
  EXPECT_GT(_POSIX_TRACE_NAME_MAX, 0);
  EXPECT_GT(_POSIX_TRACE_SYS_MAX, 0);

#if defined(__LP64__)
  EXPECT_EQ(-1, _POSIX_V7_ILP32_OFF32);
  EXPECT_EQ(-1, _POSIX_V7_ILP32_OFFBIG);
  EXPECT_GT(_POSIX_V7_LP64_OFF64, 0);
  EXPECT_GT(_POSIX_V7_LPBIG_OFFBIG, 0);
#else
  EXPECT_GT(_POSIX_V7_ILP32_OFF32, 0);
  EXPECT_EQ(-1, _POSIX_V7_ILP32_OFFBIG);
  EXPECT_EQ(-1, _POSIX_V7_LP64_OFF64);
  EXPECT_EQ(-1, _POSIX_V7_LPBIG_OFFBIG);
#endif
  ...
  EXPECT_EQ(-1, _POSIX2_PBS);
  EXPECT_EQ(-1, _POSIX2_PBS_ACCOUNTING);
  EXPECT_EQ(-1, _POSIX2_PBS_CHECKPOINT);
  EXPECT_EQ(-1, _POSIX2_PBS_LOCATE);
  EXPECT_EQ(-1, _POSIX2_PBS_MESSAGE);
  EXPECT_EQ(-1, _POSIX2_PBS_TRACK);
  EXPECT_EQ(-1, _POSIX2_SW_DEV);
  EXPECT_EQ(-1, _POSIX2_UPE);

  EXPECT_GT(_XOPEN_NAME_MAX, 0);
  EXPECT_GT(_XOPEN_PATH_MAX, 0);
...
TEST(unistd, sysconf) {
  ...
  // Some options like _SC_ADVISORY_INFO are not supported on bionic yet.
  // Some options like _SC_SS_REPL_MAX are not supported on glibc.
