...
#if defined(__BIONIC__)
  // Make a tight verification of _POSIX_* / _POSIX2_* / _XOPEN_* macros in bionic, to prevent change by mistake.
  // Verify according to POSIX 2008(2013 Edition).
  ASSERT_EQ(200809L, _POSIX_VERSION);
  ASSERT_EQ(-1, _POSIX_ADVISORY_INFO);
  ASSERT_GT(_POSIX_AIO_LISTIO_MAX, 0);
  ASSERT_GT(_POSIX_AIO_MAX, 0);
  ASSERT_GT(_POSIX_ARG_MAX, 0);
  ASSERT_EQ(-1, _POSIX_ASYNCHRONOUS_IO);
  ASSERT_EQ(-1, _POSIX_BARRIERS);
  ASSERT_GT(_POSIX_CHILD_MAX, 0);
  ASSERT_GT(_POSIX_CHOWN_RESTRICTED, 0);
  ASSERT_EQ(_POSIX_VERSION, _POSIX_CLOCK_SELECTION);
  ASSERT_EQ(-1, _POSIX_CPUTIME);
  ASSERT_GT(_POSIX_DELAYTIMER_MAX, 0);
  ASSERT_EQ(_POSIX_VERSION, _POSIX_FSYNC);
  ASSERT_GT(_POSIX_HOST_NAME_MAX, 0);
  ASSERT_EQ(_POSIX_VERSION, _POSIX_IPV6);
  ASSERT_GT(_POSIX_JOB_CONTROL, 0);
  ASSERT_GT(_POSIX_LINK_MAX, 0);
  ASSERT_GT(_POSIX_LOGIN_NAME_MAX, 0);
  ASSERT_EQ(_POSIX_VERSION, _POSIX_MAPPED_FILES);
  ASSERT_GT(_POSIX_MAX_CANON, 0);
  ASSERT_GT(_POSIX_MAX_INPUT, 0);
  ASSERT_EQ(_POSIX_VERSION, _POSIX_MEMLOCK);
  ASSERT_EQ(_POSIX_VERSION, _POSIX_MEMLOCK_RANGE);
  ASSERT_EQ(_POSIX_VERSION, _POSIX_MEMORY_PROTECTION);
  ASSERT_EQ(-1, _POSIX_MESSAGE_PASSING);
  ASSERT_EQ(0, _POSIX_MONOTONIC_CLOCK);
  ASSERT_GT(_POSIX_MQ_OPEN_MAX, 0);
  ASSERT_GT(_POSIX_MQ_PRIO_MAX, 0);
  ASSERT_GT(_POSIX_NAME_MAX, 0);
  ASSERT_GT(_POSIX_NGROUPS_MAX, 0);
  ASSERT_GT(_POSIX_NO_TRUNC, 0);
  ASSERT_GT(_POSIX_OPEN_MAX, 0);
  ASSERT_GT(_POSIX_PATH_MAX, 0);
  ASSERT_GT(_POSIX_PIPE_BUF, 0);
  ASSERT_EQ(_POSIX_VERSION, _POSIX_PRIORITY_SCHEDULING);
  ASSERT_EQ(-1, _POSIX_PRIORITIZED_IO);
  ASSERT_EQ(_POSIX_VERSION, _POSIX_RAW_SOCKETS);
  ASSERT_EQ(_POSIX_VERSION, _POSIX_READER_WRITER_LOCKS);
  ASSERT_EQ(-1, _POSIX_REALTIME_SIGNALS);
  ASSERT_GT(_POSIX_REGEXP, 0);
  ASSERT_GT(_POSIX_RE_DUP_MAX, 0);
  ASSERT_GT(_POSIX_SAVED_IDS, 0);
  ASSERT_EQ(_POSIX_VERSION, _POSIX_SEMAPHORES);
  ASSERT_GT(_POSIX_SEM_NSEMS_MAX, 0);
  ASSERT_GT(_POSIX_SEM_VALUE_MAX, 0);
  ASSERT_EQ(-1, _POSIX_SHARED_MEMORY_OBJECTS);
  ASSERT_GT(_POSIX_SHELL, 0);
  ASSERT_GT(_POSIX_SIGQUEUE_MAX, 0);
  ASSERT_EQ(-1, _POSIX_SPAWN);
  ASSERT_EQ(-1, _POSIX_SPIN_LOCKS);
  ASSERT_EQ(-1, _POSIX_SPORADIC_SERVER);
  ASSERT_GT(_POSIX_SSIZE_MAX, 0);
  ASSERT_GT(_POSIX_SS_REPL_MAX, 0);
  ASSERT_GT(_POSIX_STREAM_MAX, 0);
  ASSERT_GT(_POSIX_SYMLINK_MAX, 0);
  ASSERT_GT(_POSIX_SYMLOOP_MAX, 0);
  ASSERT_EQ(_POSIX_VERSION, _POSIX_SYNCHRONIZED_IO);
  ASSERT_EQ(_POSIX_VERSION, _POSIX_THREADS);
  ASSERT_EQ(_POSIX_VERSION, _POSIX_THREAD_ATTR_STACKADDR);
  ASSERT_EQ(_POSIX_VERSION, _POSIX_THREAD_ATTR_STACKSIZE);
  ASSERT_EQ(_POSIX_VERSION, _POSIX_THREAD_CPUTIME);
  ASSERT_GT(_POSIX_THREAD_DESTRUCTOR_ITERATIONS, 0);
  ASSERT_GT(_POSIX_THREAD_KEYS_MAX, 0);
  ASSERT_EQ(_POSIX_VERSION, _POSIX_THREAD_PRIORITY_SCHEDULING);
  ASSERT_EQ(_POSIX_VERSION, _POSIX_THREAD_PRIO_INHERIT);
  ASSERT_EQ(_POSIX_VERSION, _POSIX_THREAD_PRIO_PROTECT);
  ASSERT_EQ(-1, _POSIX_THREAD_PROCESS_SHARED);
  ASSERT_EQ(-1, _POSIX_THREAD_ROBUST_PRIO_INHERIT);
  ASSERT_EQ(-1, _POSIX_THREAD_ROBUST_PRIO_PROTECT);
  ASSERT_EQ(_POSIX_VERSION, _POSIX_THREAD_SAFE_FUNCTIONS);
  ASSERT_EQ(-1, _POSIX_THREAD_SPORADIC_SERVER);
  ASSERT_GT(_POSIX_THREAD_THREADS_MAX, 0);
  ASSERT_EQ(_POSIX_VERSION, _POSIX_TIMEOUTS);
  ASSERT_EQ(_POSIX_VERSION, _POSIX_TIMERS);
  ASSERT_GT(_POSIX_TIMER_MAX, 0);
  ASSERT_EQ(-1, _POSIX_TRACE);
  ASSERT_EQ(-1, _POSIX_TRACE_EVENT_FILTER);
  ASSERT_GT(_POSIX_TRACE_EVENT_NAME_MAX, 0);
  ASSERT_EQ(-1, _POSIX_TRACE_INHERIT);
  ASSERT_EQ(-1, _POSIX_TRACE_LOG);
  ASSERT_GT(_POSIX_TRACE_NAME_MAX, 0);
  ASSERT_GT(_POSIX_TRACE_SYS_MAX, 0);
  ASSERT_GT(_POSIX_TTY_NAME_MAX, 0);
  ASSERT_EQ(-1, _POSIX_TYPED_MEMORY_OBJECTS);
  ASSERT_GT(_POSIX_TZNAME_MAX, 0);
  ASSERT_NE(-1, _POSIX_VDISABLE);

#if defined(__LP64__)
  ASSERT_EQ(-1, _POSIX_V7_ILP32_OFF32);
  ASSERT_EQ(-1, _POSIX_V7_ILP32_OFFBIG);
  ASSERT_GT(_POSIX_V7_LP64_OFF64, 0);
  ASSERT_GT(_POSIX_V7_LPBIG_OFFBIG, 0);
#else
  ASSERT_GT(_POSIX_V7_ILP32_OFF32, 0);
  ASSERT_EQ(-1, _POSIX_V7_ILP32_OFFBIG);
  ASSERT_EQ(-1, _POSIX_V7_LP64_OFF64);
  ASSERT_EQ(-1, _POSIX_V7_LPBIG_OFFBIG);
#endif

  ASSERT_EQ(-1, _POSIX2_VERSION);
  ASSERT_GT(_POSIX2_BC_BASE_MAX, 0);
  ASSERT_GT(_POSIX2_BC_DIM_MAX, 0);
  ASSERT_GT(_POSIX2_BC_SCALE_MAX, 0);
  ASSERT_GT(_POSIX2_BC_STRING_MAX, 0);
  ASSERT_GT(_POSIX2_CHARCLASS_NAME_MAX, 0);
  ASSERT_EQ(-1, _POSIX2_CHAR_TERM);
  ASSERT_GT(_POSIX2_COLL_WEIGHTS_MAX, 0);
  ASSERT_EQ(_POSIX_VERSION, _POSIX2_C_BIND);
  ASSERT_EQ(-1, _POSIX2_C_DEV);
  ASSERT_GT(_POSIX2_EXPR_NEST_MAX, 0);
  ASSERT_EQ(-1, _POSIX2_FORT_DEV);
  ASSERT_EQ(-1, _POSIX2_FORT_RUN);
  ASSERT_GT(_POSIX2_LINE_MAX, 0);
  ASSERT_EQ(-1, _POSIX2_LOCALEDEF);
  ASSERT_EQ(-1, _POSIX2_PBS);
  ASSERT_EQ(-1, _POSIX2_PBS_ACCOUNTING);
  ASSERT_EQ(-1, _POSIX2_PBS_CHECKPOINT);
  ASSERT_EQ(-1, _POSIX2_PBS_LOCATE);
  ASSERT_EQ(-1, _POSIX2_PBS_MESSAGE);
  ASSERT_EQ(-1, _POSIX2_PBS_TRACK);
  ASSERT_GT(_POSIX2_RE_DUP_MAX, 0);
  ASSERT_EQ(-1, _POSIX2_SW_DEV);
  ASSERT_EQ(-1, _POSIX2_UPE);

  ASSERT_EQ(700, _XOPEN_VERSION);
  ASSERT_EQ(-1, _XOPEN_ENH_I18N);
  ASSERT_EQ(-1, _XOPEN_CRYPT);
  ASSERT_GT(_XOPEN_IOV_MAX, 0);
  ASSERT_EQ(-1, _XOPEN_LEGACY);
  ASSERT_GT(_XOPEN_NAME_MAX, 0);
  ASSERT_GT(_XOPEN_PATH_MAX, 0);
  ASSERT_EQ(-1, _XOPEN_REALTIME);
  ASSERT_EQ(-1, _XOPEN_REALTIME_THREADS);
  ASSERT_EQ(-1, _XOPEN_SHM);
  ASSERT_EQ(-1, _XOPEN_STREAMS);
  ASSERT_GT(_XOPEN_UNIX, 0);
  ASSERT_EQ(-1, _XOPEN_UUCP);

#else // !defined(__BIONIC__)
  GTEST_LOG_(INFO) << "The test is only used to verify _POSIX_* support in bionic.\n";
#endif
}

#if defined(__BIONIC__)

#define VERIFY_SYSCONF_MINUS_ONE(name) VerifySysconf(name, #name, [](long v){return v == -1;})
#define VERIFY_SYSCONF_POSITIVE(name) VerifySysconf(name, #name, [](long v){return v > 0;})
#define VERIFY_SYSCONF_POSIX_VERSION(name) VerifySysconf(name, #name, [](long v){return v == _POSIX_VERSION;})

static void VerifySysconf(int option, const char *option_name, bool (*verify)(long)) {
  errno = 0;
  long ret = sysconf(option);
  ASSERT_TRUE(0 == errno && verify(ret)) << "name = " << option_name << ", ret = "
...
TEST(unistd, sysconf) {
  ...
  VERIFY_SYSCONF_MINUS_ONE(_SC_2_C_DEV);
  VERIFY_SYSCONF_MINUS_ONE(_SC_2_CHAR_TERM);
  VERIFY_SYSCONF_MINUS_ONE(_SC_2_FORT_DEV);
  VERIFY_SYSCONF_MINUS_ONE(_SC_2_FORT_RUN);
  VERIFY_SYSCONF_MINUS_ONE(_SC_2_LOCALEDEF);
  VERIFY_SYSCONF_MINUS_ONE(_SC_2_SW_DEV);
  VERIFY_SYSCONF_MINUS_ONE(_SC_2_UPE);
  VERIFY_SYSCONF_MINUS_ONE(_SC_2_VERSION);
  ...
  VERIFY_SYSCONF_MINUS_ONE(_SC_XOPEN_CRYPT);
  VERIFY_SYSCONF_MINUS_ONE(_SC_XOPEN_ENH_I18N);
  VERIFY_SYSCONF_MINUS_ONE(_SC_XOPEN_SHM);
  VerifySysconf(_SC_XOPEN_VERSION, "_SC_XOPEN_VERSION", [](long v){return v == _XOPEN_VERSION;});
  VERIFY_SYSCONF_MINUS_ONE(_SC_XOPEN_REALTIME);
  VERIFY_SYSCONF_MINUS_ONE(_SC_XOPEN_REALTIME_THREADS);
  VERIFY_SYSCONF_MINUS_ONE(_SC_XOPEN_LEGACY);
  ...
  VERIFY_SYSCONF_MINUS_ONE(_SC_ASYNCHRONOUS_IO);
  ...
  VERIFY_SYSCONF_MINUS_ONE(_SC_MESSAGE_PASSING);
  VERIFY_SYSCONF_MINUS_ONE(_SC_PRIORITIZED_IO);
  VERIFY_SYSCONF_POSIX_VERSION(_SC_PRIORITY_SCHEDULING);
  VERIFY_SYSCONF_MINUS_ONE(_SC_REALTIME_SIGNALS);
  VERIFY_SYSCONF_POSIX_VERSION(_SC_SEMAPHORES);
  VERIFY_SYSCONF_MINUS_ONE(_SC_SHARED_MEMORY_OBJECTS);
  ...
  VERIFY_SYSCONF_MINUS_ONE(_SC_2_PBS);
  VERIFY_SYSCONF_MINUS_ONE(_SC_2_PBS_ACCOUNTING);
  VERIFY_SYSCONF_MINUS_ONE(_SC_2_PBS_CHECKPOINT);
  VERIFY_SYSCONF_MINUS_ONE(_SC_2_PBS_LOCATE);
  VERIFY_SYSCONF_MINUS_ONE(_SC_2_PBS_MESSAGE);
  VERIFY_SYSCONF_MINUS_ONE(_SC_2_PBS_TRACK);
  VERIFY_SYSCONF_MINUS_ONE(_SC_ADVISORY_INFO);
  VERIFY_SYSCONF_MINUS_ONE(_SC_BARRIERS);
  VERIFY_SYSCONF_POSIX_VERSION(_SC_CLOCK_SELECTION);
  VERIFY_SYSCONF_MINUS_ONE(_SC_CPUTIME);
  ...
  VERIFY_SYSCONF_MINUS_ONE(_SC_SPAWN);
  VERIFY_SYSCONF_MINUS_ONE(_SC_SPIN_LOCKS);
  VERIFY_SYSCONF_MINUS_ONE(_SC_SPORADIC_SERVER);
  VERIFY_SYSCONF_POSITIVE(_SC_SS_REPL_MAX);
  VERIFY_SYSCONF_POSITIVE(_SC_SYMLOOP_MAX);
  VERIFY_SYSCONF_POSIX_VERSION(_SC_THREAD_CPUTIME);
  VERIFY_SYSCONF_MINUS_ONE(_SC_THREAD_PROCESS_SHARED);
  VERIFY_SYSCONF_MINUS_ONE(_SC_THREAD_ROBUST_PRIO_INHERIT);
  VERIFY_SYSCONF_MINUS_ONE(_SC_THREAD_ROBUST_PRIO_PROTECT);
  VERIFY_SYSCONF_MINUS_ONE(_SC_THREAD_SPORADIC_SERVER);
  VERIFY_SYSCONF_POSIX_VERSION(_SC_TIMEOUTS);
  VERIFY_SYSCONF_MINUS_ONE(_SC_TRACE);
  VERIFY_SYSCONF_MINUS_ONE(_SC_TRACE_EVENT_FILTER);
  VERIFY_SYSCONF_POSITIVE(_SC_TRACE_EVENT_NAME_MAX);
  VERIFY_SYSCONF_MINUS_ONE(_SC_TRACE_INHERIT);
  VERIFY_SYSCONF_MINUS_ONE(_SC_TRACE_LOG);
  VERIFY_SYSCONF_POSITIVE(_SC_TRACE_NAME_MAX);
  VERIFY_SYSCONF_POSITIVE(_SC_TRACE_SYS_MAX);
  VERIFY_SYSCONF_POSITIVE(_SC_TRACE_USER_EVENT_MAX);
  VERIFY_SYSCONF_MINUS_ONE(_SC_TYPED_MEMORY_OBJECTS);
#if defined(__LP64__)
  VERIFY_SYSCONF_MINUS_ONE(_SC_V7_ILP32_OFF32);
  VERIFY_SYSCONF_MINUS_ONE(_SC_V7_ILP32_OFFBIG);
  ...
  VERIFY_SYSCONF_MINUS_ONE(_SC_V7_ILP32_OFFBIG);
  VERIFY_SYSCONF_MINUS_ONE(_SC_V7_LP64_OFF64);
  VERIFY_SYSCONF_MINUS_ONE(_SC_V7_LPBIG_OFFBIG);
#endif
  VERIFY_SYSCONF_MINUS_ONE(_SC_XOPEN_STREAMS);
  VERIFY_SYSCONF_MINUS_ONE(_SC_XOPEN_UUCP);

#else // !defined(__BIONIC__)
  GTEST_LOG_(INFO) << "The test is only used to verify sysconf() support in bionic.\n";
#endif
}
