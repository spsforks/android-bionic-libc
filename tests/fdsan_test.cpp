...
#define EXPECT_FDSAN_DEATH(expression, regex) \
  EXPECT_DEATH((android_fdsan_set_enabled(true), expression), (regex))
...
#if defined(__BIONIC__)
  int fd = open("/dev/null", O_RDONLY);
  EXPECT_FDSAN_DEATH(android_fdsan_close_with_tag(fd, 0xdeadbeef), "incorrect tag");
#endif
}
...
#if defined(__BIONIC__)
  int fd = open("/dev/null", O_RDONLY);
  EXPECT_FDSAN_DEATH(android_fdsan_exchange_owner_tag(fd, 0xbadc0de, 0xdeadbeef), "failed to exchange");
#endif
}
...
TEST_F(FdsanTest, owned_untagged_close) {
  ...
  EXPECT_FDSAN_DEATH(close(fd), "incorrect tag");
#endif
}
...
TEST_F(FdsanTest, owned_improperly_tagged_close) {
  ...
  int fd = open("/dev/null", O_RDONLY);
  android_fdsan_exchange_owner_tag(fd, 0, 0xdeadbeef);
  EXPECT_FDSAN_DEATH(android_fdsan_close_with_tag(fd, 0xdeadc0de), "incorrect tag");
#endif
}
...
TEST_F(FdsanTest, fopen) {
  ...
  ASSERT_TRUE(f);
  EXPECT_FDSAN_DEATH(close(fileno(f)), "incorrect tag");
#endif
}
...
TEST_F(FdsanTest, closedir) {
  ...
  DIR* dir = opendir("/dev/");
  ASSERT_TRUE(dir);
  EXPECT_FDSAN_DEATH(close(dirfd(dir)), "incorrect tag");
#endif
}
