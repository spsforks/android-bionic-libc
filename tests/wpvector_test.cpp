...
TEST(wpvector, wpvector_resize) {
  ...
  ASSERT_EQ(page_size, wpvec.header->mappingSize);
  ASSERT_EQ('a', vec[0]);
...
TEST(wpvector, wpvector_large) {
  ...
  auto vec = wpvec.mutate();
  vec.resize(1);
  ASSERT_EQ(vec.header->mappingSize, page_size * (struct_pages + 1));
  vec.resize(2);
  ASSERT_EQ(vec.header->mappingSize, page_size * (struct_pages * 2));
  vec.resize(3);
  ASSERT_EQ(vec.header->mappingSize, page_size * (struct_pages * 3));
}
...
#define EPILOGUE                                                         \
      EXPECT_EQ(page_size* expected_pages, wpvec.header->mappingSize);   \
...
TEST(wpvector, wpvector_realloc_b) {
  PROLOGUE(page_size - sizeof(WPVectorHeader) - 1)
    ...
    ASSERT_EQ(page_size, vec.header->mappingSize);
