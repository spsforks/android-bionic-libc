...
TEST(STDIO_TEST, wprintf_m) {
  ...
  const std::wstring kExpPrefix = L"<Invalid argument";
  const std::wstring kExpSuffix = L">";
  const std::wstring wsBuf(buf);
  ASSERT_EQ(wsBuf.substr(0, kExpPrefix.size()), kExpPrefix);
  ASSERT_GE(wsBuf.size(), kExpSuffix.size());
  ASSERT_EQ(wsBuf.substr(wsBuf.size() - kExpSuffix.size(), kExpSuffix.size()), kExpSuffix);
}
...
TEST(STDIO_TEST, perror) {
  ExecTestHelper eth;
  eth.Run([&]() { errno = EINVAL; perror("a b c"); exit(0); }, 0, nullptr);
  ASSERT_MATCH(eth.GetOutput(), "a b c: Invalid argument.*\n");
  eth.Run([&]() { errno = EINVAL; perror(nullptr); exit(0); }, 0, nullptr);
  ASSERT_MATCH(eth.GetOutput(), "Invalid argument.*\n");
  eth.Run([&]() { errno = EINVAL; perror(""); exit(0); }, 0, nullptr);
  ASSERT_MATCH(eth.GetOutput(), "Invalid argument.*\n");
}
