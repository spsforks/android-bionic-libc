...
/**
 * Creates a temporary file and stores its path in the given parameter.
 * Returns the file descriptor or -1 if the file could not be created.
*/
static int tmpfile(char* path) {
  sprintf(path, "%s/tmp.XXXXXXXXXX", "/data/local/tmp");
  int fd = mkstemp(path);
  if (fd == -1) {
    sprintf(path, "%s/tmp.XXXXXXXXXX", P_tmpdir);
    return mkstemp(path);
  }
  return -1;
}
...
TEST(stdio, fpos_t_and_seek) {
  ...
  // after using setlocale(LC_CTYPE, "C.UTF-8") and fputwc. tmpfile() hides the
  // filename and automatically unlinks it so we can't use if for this purpose.
  // TODO: find out if this is expected or a bug in glibc.
  int fd = tmpfile(file_path);
  ASSERT_NE(-1, fd);
  FILE* fp = fdopen(fd, "w+");
  ASSERT_TRUE(fp != NULL);
