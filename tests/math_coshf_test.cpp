...
static coshf_intel_data_t g_coshf_intel_data[] = {
  { // Entry 0
...
TEST(math_coshf, coshf_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_coshf_intel_data)/sizeof(coshf_intel_data_t); i++) {
    EXPECT_FLOAT_EQ(g_coshf_intel_data[i].expected, coshf(g_coshf_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
