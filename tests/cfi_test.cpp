...
class cfi_test_DeathTest : public testing::Test {
 protected:
  void SetUp() override {
    ...
    sigaction64(SIGSEGV, &action, &previous_);
    sigaction64(SIGILL, &action, &previous_);
  }

  void TearDown() override {
    sigaction64(SIGILL, &previous_, nullptr);
    sigaction64(SIGSEGV, &previous_, nullptr);
  }
...
#undef SYM
  ...
  EXPECT_EXIT(test_cfi_slowpath_with_alloc(), testing::KilledBySignal(SIGSEGV), "");
  ...
  // killed.
  EXPECT_EXIT(__cfi_slowpath(45, reinterpret_cast<void*>(code_ptr)),
