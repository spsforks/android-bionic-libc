...
static modf_intel_data_t g_modf_intel_data[] = {
  { // Entry 0
    -0x1.p-1074,
    -0.0,
    -0x1.0p-1074
...
TEST(math_modf, modf_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_modf_intel_data)/sizeof(modf_intel_data_t); i++) {
    double dintegral;
    EXPECT_DOUBLE_EQ(g_modf_intel_data[i].expected, modf(g_modf_intel_data[i].call_data, &dintegral)) << "Failed on element " << i;
    EXPECT_DOUBLE_EQ(g_modf_intel_data[i].integral_expected, dintegral) << "Failed on element " << i;
  }
#else // __BIONIC__
