...
static void print_ll(const char* what, sockaddr* p) {
  sockaddr_ll* s = reinterpret_cast<sockaddr_ll*>(p);
  printf("\t%s\t", what);
  for (int i = 0; i < s->sll_halen; i++) {
    printf("%02X%s",s->sll_addr[i],i < (s->sll_halen-1) ? ":":"");
  }
  printf(" (%d bytes)\n", s->sll_halen);
}

static void print_inet(const char* what, sockaddr* addr) {
...
TEST(ifaddrs, dump) {
  ...
  for (ifaddrs* ifa = addrs; ifa != nullptr; ifa = ifa->ifa_next) {
    ...
    if (family == AF_PACKET) {
      if (ifa->ifa_addr) print_ll("hwaddr", ifa->ifa_addr);
      if (ifa->ifa_broadaddr) print_ll("hwbroad", ifa->ifa_addr);
    } else if (family == AF_INET || family == AF_INET6) {
      if (ifa->ifa_addr) print_inet("address", ifa->ifa_addr);
      if (ifa->ifa_broadaddr && (ifa->ifa_flags & (IFF_BROADCAST | IFF_POINTOPOINT)) != 0) {
        print_inet((ifa->ifa_flags & IFF_BROADCAST) ? "broadcast" : "destination",
                   ifa->ifa_broadaddr);
      }
    }
