...
static sinhf_intel_data_t g_sinhf_intel_data[] = {
  { // Entry 0
...
TEST(math_sinhf, sinhf_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_sinhf_intel_data)/sizeof(sinhf_intel_data_t); i++) {
    EXPECT_FLOAT_EQ(g_sinhf_intel_data[i].expected, sinhf(g_sinhf_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
