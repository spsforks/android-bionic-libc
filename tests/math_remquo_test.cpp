...
static remquo_intel_data_t g_remquo_intel_data[] = {
  { // Entry 0
...
TEST(math_remquo, remquo_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_remquo_intel_data)/sizeof(remquo_intel_data_t); i++) {
    int iquo;
    EXPECT_DOUBLE_EQ(g_remquo_intel_data[i].expected, remquo(g_remquo_intel_data[i].call_data1, g_remquo_intel_data[i].call_data2, &iquo)) << "Failed on element " << i;
    EXPECT_EQ(g_remquo_intel_data[i].quo_expected, iquo) << "Failed on element " << i;
  }
#else // __BIONIC__
