...
static frexp_intel_data_t g_frexp_intel_data[] = {
  { // Entry 0
    0x1.p-1,
    (int)0x1.94p6,
    0x1.0p100
...
TEST(math_frexp, frexp_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_frexp_intel_data)/sizeof(frexp_intel_data_t); i++) {
    int iexp;
    EXPECT_DOUBLE_EQ(g_frexp_intel_data[i].expected, frexp(g_frexp_intel_data[i].call_data, &iexp)) << "Failed on element " << i;
    EXPECT_EQ(g_frexp_intel_data[i].exp_expected, iexp) << "Failed on element " << i;
  }
#else // __BIONIC__
