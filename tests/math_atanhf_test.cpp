...
static atanhf_intel_data_t g_atanhf_intel_data[] = {
  { // Entry 0
...
TEST(math_atanhf, atanhf_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_atanhf_intel_data)/sizeof(atanhf_intel_data_t); i++) {
    EXPECT_FLOAT_EQ(g_atanhf_intel_data[i].expected, atanhf(g_atanhf_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
