...
static fdimf_intel_data_t g_fdimf_intel_data[] = {
  { // Entry 0
    0x1.861861p-14,
    -0x1.86187ep-15,
    -0x1.249250p-13
...
TEST(math_fdimf, fdimf_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_fdimf_intel_data)/sizeof(fdimf_intel_data_t); i++) {
    EXPECT_FLOAT_EQ(g_fdimf_intel_data[i].expected, fdimf(g_fdimf_intel_data[i].call_data1, g_fdimf_intel_data[i].call_data2)) << "Failed on element " << i;
  }
#else // __BIONIC__
