...
TEST(sched, clone) {
  ...
  clone_test(NULL);
#else
  // There is a bug that causes pthread tests to fail when clone afterwards.
  // For now fork off a process to allow the test to complete properly. Making
  // this a DeathTest can work, but loses fidelity about what failed.
  pid_t pid;
  if ((pid = fork()) == 0) {
    int exit_code = 0;
