...
static sqrtf_intel_data_t g_sqrtf_intel_data[] = {
  { // Entry 0
    0x1.p-1,
    0x1.p-2
  },
...
TEST(math_sqrtf, sqrtf_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_sqrtf_intel_data)/sizeof(sqrtf_intel_data_t); i++) {
    EXPECT_FLOAT_EQ(g_sqrtf_intel_data[i].expected, sqrtf(g_sqrtf_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
