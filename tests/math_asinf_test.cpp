...
static asinf_intel_data_t g_asinf_intel_data[] = {
  { // Entry 0
    -0x1.p-149,
    -0x1.p-149
  },
...
TEST(math_asinf, asinf_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_asinf_intel_data)/sizeof(asinf_intel_data_t); i++) {
    EXPECT_FLOAT_EQ(g_asinf_intel_data[i].expected, asinf(g_asinf_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
