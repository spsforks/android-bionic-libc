...
static logb_intel_data_t g_logb_intel_data[] = {
  { // Entry 0
    -0x1.0c80p10,
    -0x1.0p-1074
  },
...
TEST(math_logb, logb_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_logb_intel_data)/sizeof(logb_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_logb_intel_data[i].expected, logb(g_logb_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
