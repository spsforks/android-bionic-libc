...
static sinh_intel_data_t g_sinh_intel_data[] = {
  { // Entry 0
...
TEST(math_sinh, sinh_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_sinh_intel_data)/sizeof(sinh_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_sinh_intel_data[i].expected, sinh(g_sinh_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
