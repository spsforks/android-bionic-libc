...
static void test_ld_library_path_exec_program(const std::string& program,
  ...
  if (child_pid == 0) {
    ...
    ASSERT_TRUE(WIFEXITED(status)) << "Child process did not terminate normally";

    static char err_output[MAX_ERROR_LENGTH];
    ssize_t size_read = 0;
    ssize_t tmp;
    while ((tmp = read(pipefd[0], err_output + size_read, MAX_ERROR_LENGTH - size_read)) > 0) {
      size_read += tmp;
    }
    EXPECT_GE(tmp, 0);  // Ensure that no error occurred (negative return from read()).
    err_output[size_read] = '\0';
    close(pipefd[0]);

    ASSERT_EQ(WEXITSTATUS(status) == 0, expected_error.empty()) << err_output
        << " (LD_LIBRARY_PATH = " << lib_dir << ")";
    if (expected_error.empty()) {
      ASSERT_EQ(size_read, 0) << "LD_LIBRARY_PATH = " << lib_dir;
    } else {
      ASSERT_SUBSTR(expected_error, err_output) << "LD_LIBRARY_PATH = " << lib_dir;
    }
  }
}
