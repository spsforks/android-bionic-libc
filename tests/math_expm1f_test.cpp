...
static expm1f_intel_data_t g_expm1f_intel_data[] = {
  { // Entry 0
...
TEST(math_expm1f, expm1f_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_expm1f_intel_data)/sizeof(expm1f_intel_data_t); i++) {
    EXPECT_FLOAT_EQ(g_expm1f_intel_data[i].expected, expm1f(g_expm1f_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
