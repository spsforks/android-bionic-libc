...
static fma_intel_data_t g_fma_intel_data[] = {
  { // Entry 0
...
TEST(math_fma, fma_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_fma_intel_data)/sizeof(fma_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_fma_intel_data[i].expected, fma(g_fma_intel_data[i].call_data1, g_fma_intel_data[i].call_data2, g_fma_intel_data[i].call_data3)) << "Failed on element " << i;
  }
#else // __BIONIC__
