...
void* is_ctor_called_ifun() {
  return g_flag == 0 ? reinterpret_cast<void*>(return_false) : reinterpret_cast<void*>(return_true);
}

void* foo_ifunc() {
  char* choice = getenv("IFUNC_CHOICE");
  return choice == NULL ? reinterpret_cast<void*>(f1) : reinterpret_cast<void*>(f2);
}
