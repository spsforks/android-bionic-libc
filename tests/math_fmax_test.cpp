...
static fmax_intel_data_t g_fmax_intel_data[] = {
  { // Entry 0
    -0x1.40p3,
    -0x1.4p3,
    -0x1.4p3
...
TEST(math_fmax, fmax_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_fmax_intel_data)/sizeof(fmax_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_fmax_intel_data[i].expected, fmax(g_fmax_intel_data[i].call_data1, g_fmax_intel_data[i].call_data2)) << "Failed on element " << i;
  }
#else // __BIONIC__
