...
static log10f_intel_data_t g_log10f_intel_data[] = {
  { // Entry 0
...
TEST(math_log10f, log10f_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_log10f_intel_data)/sizeof(log10f_intel_data_t); i++) {
    EXPECT_FLOAT_EQ(g_log10f_intel_data[i].expected, log10f(g_log10f_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
