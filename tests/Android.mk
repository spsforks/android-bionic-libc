...
# -----------------------------------------------------------------------------
$(eval \
  $(foreach compiler,gcc compiler, \
    $(foreach test,1 2, \
      fortify$(test)-tests-$(compiler)_cflags := \
        $(test_cflags) \
        -U_FORTIFY_SOURCE \
        -D_FORTIFY_SOURCE=$(test) \
        -DTEST_NAME=Fortify$(test)_$(compiler); \
      fortify$(test)-tests-$(compiler)_cflags_host := \
        -Wno-error; \
      fortify$(test)-tests-$(compiler)_src_files := \
        fortify_test.cpp; \
      fortify_libs := \
        fortify$(test)-tests-$(compiler); \
    ) \
  ) \
)
