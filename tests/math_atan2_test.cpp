...
static atan2_intel_data_t g_atan2_intel_data[] = {
  { // Entry 0
...
TEST(math_atan2, atan2_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_atan2_intel_data)/sizeof(atan2_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_atan2_intel_data[i].expected, atan2(g_atan2_intel_data[i].call_data1, g_atan2_intel_data[i].call_data2)) << "Failed on element " << i;
  }
#else // __BIONIC__
