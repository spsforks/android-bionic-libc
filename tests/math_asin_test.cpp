...
static asin_intel_data_t g_asin_intel_data[] = {
  { // Entry 0
...
TEST(math_asin, asin_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_asin_intel_data)/sizeof(asin_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_asin_intel_data[i].expected, asin(g_asin_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
