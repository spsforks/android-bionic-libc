...
static hypotf_intel_data_t g_hypotf_intel_data[] = {
  { // Entry 0
...
TEST(math_hypotf, hypotf_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_hypotf_intel_data)/sizeof(hypotf_intel_data_t); i++) {
    EXPECT_FLOAT_EQ(g_hypotf_intel_data[i].expected, hypotf(g_hypotf_intel_data[i].call_data1, g_hypotf_intel_data[i].call_data2)) << "Failed on element " << i;
  }
#else // __BIONIC__
