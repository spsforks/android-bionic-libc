...
static log2f_intel_data_t g_log2f_intel_data[] = {
  { // Entry 0
...
TEST(math_log2f, log2f_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_log2f_intel_data)/sizeof(log2f_intel_data_t); i++) {
    EXPECT_FLOAT_EQ(g_log2f_intel_data[i].expected, log2f(g_log2f_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
