...
TEST(atexit, combined_test) {
  std::string control_atexit_sequence;
  ...
  reinterpret_cast<void (*)(std::string*, int*)>(sym)(&control_atexit_sequence, &control_staticdtors);

  ASSERT_EQ(0, dlclose(handle));
  // this test verifies atexit call from atexit handler. as well as the order of calls
  ASSERT_STREQ("Humpty Dumpty sat on a wall", control_atexit_sequence.c_str());
