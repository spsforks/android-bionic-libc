...
static acosh_intel_data_t g_acosh_intel_data[] = {
  { // Entry 0
...
TEST(math_acosh, acosh_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_acosh_intel_data)/sizeof(acosh_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_acosh_intel_data[i].expected, acosh(g_acosh_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
