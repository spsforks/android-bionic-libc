...
TEST(sys_queue, single_list)
{
  ...
  { SCOPED_TRACE("check init state");
    ASSERT_NE( SLIST_EMPTY(&nodeHead), 0 );
    ASSERT_EQ( NULL, SLIST_FIRST(&nodeHead) );
  }

  { SCOPED_TRACE("inserts");
    // = Node 1
    node1 = (slEntry *) malloc(sizeof(struct slEntry));
    ASSERT_NE( node1, static_cast <void*> (NULL) );
    ...
    ASSERT_NE( node2, static_cast <void*> (NULL) );
    ...
    ASSERT_NE( node3, static_cast <void*> (NULL) );
  ...
  { SCOPED_TRACE("inject");
    ...
    ASSERT_NE( node12, static_cast <void*> (NULL) );
  ...
  { SCOPED_TRACE("check relations");
    EXPECT_EQ( node1, SLIST_FIRST(&nodeHead) );
    EXPECT_EQ( node12, SLIST_NEXT(node1, entries) );
    EXPECT_EQ( NULL, SLIST_NEXT(node3, entries) );
    EXPECT_EQ( node12, SLIST_NEXT(SLIST_FIRST(&nodeHead), entries) );
  }

  { SCOPED_TRACE("remove node");
    SLIST_REMOVE(&nodeHead, node12, slEntry, entries);
    EXPECT_EQ( node2, SLIST_NEXT(SLIST_FIRST(&nodeHead), entries) );
  }

  { SCOPED_TRACE("remove head");
    SLIST_REMOVE_HEAD(&nodeHead, entries);
    EXPECT_EQ( node2, SLIST_FIRST(&nodeHead) );

    SLIST_REMOVE_HEAD(&nodeHead, entries);
    EXPECT_EQ( node3, SLIST_FIRST(&nodeHead) );
...
TEST(sys_queue, double_list)
{
  ...
  { SCOPED_TRACE("check init state");
    ASSERT_NE( LIST_EMPTY(&nodeHead), 0 );
    ASSERT_EQ( NULL, LIST_FIRST(&nodeHead) );
  }
  ...
  { SCOPED_TRACE("inject");
    ...
    { SCOPED_TRACE("check relations");
      EXPECT_EQ( node1, LIST_FIRST(&nodeHead) );
      EXPECT_EQ( node12, LIST_NEXT(node1, entries) );
      EXPECT_EQ( NULL, LIST_NEXT(node3, entries) );
      EXPECT_EQ( node12, LIST_NEXT(LIST_FIRST(&nodeHead), entries) );
    }

    { SCOPED_TRACE("remove nodes");
      LIST_REMOVE(node12, entries);
      EXPECT_EQ( node2, LIST_NEXT(LIST_FIRST(&nodeHead), entries) );

      LIST_REMOVE(node1, entries);
      EXPECT_EQ( node2, LIST_FIRST(&nodeHead) );

      LIST_REMOVE(node2, entries);
      EXPECT_EQ( node3, LIST_FIRST(&nodeHead) );
...
TEST(sys_queue, tail_queue)
{
    ...
    { SCOPED_TRACE("check init state");
      ASSERT_NE( TAILQ_EMPTY(&nodeHead), 0 );
      ASSERT_EQ( NULL, TAILQ_FIRST(&nodeHead) );
      ASSERT_EQ( NULL, TAILQ_LAST(&nodeHead, tailhead) );
    }
    ...
    { SCOPED_TRACE("inserts");
      ...
      ASSERT_NE( node4, static_cast <void*> (NULL) );
      ...
      TAILQ_FOREACH(carry, &nodeHead, entries)
	{
	  switch (counter)
	    {
	    case 0: {
	      EXPECT_EQ( node1, carry );
	      break;
	    }
	    case 1: {
	      EXPECT_EQ( node12, carry );
	      break;
	    }
	    case 2: {
	      EXPECT_EQ( node2, carry );
	      break;
	    }
	    case 3: {
	      EXPECT_EQ( node4, carry );
	      break;
	    }
	    case 4: {
	      EXPECT_EQ( node3, carry );
	      break;
	    }
	    default: {
	      FAIL();
    ...
    { SCOPED_TRACE("check relations");
      EXPECT_EQ( node1, TAILQ_FIRST(&nodeHead) );
      EXPECT_EQ( node12, TAILQ_NEXT(node1, entries) );
      EXPECT_EQ( NULL, TAILQ_NEXT(node3, entries) );
      EXPECT_EQ( node12, TAILQ_NEXT(TAILQ_FIRST(&nodeHead), entries) );
      EXPECT_EQ( NULL, TAILQ_PREV(node1, tailhead, entries) );
      EXPECT_EQ( node1, TAILQ_PREV(node12, tailhead, entries) );
      EXPECT_EQ( node4, TAILQ_PREV(TAILQ_LAST(&nodeHead, tailhead), tailhead, entries) );
    }

    { SCOPED_TRACE("remove nodes");
      TAILQ_REMOVE(&nodeHead, node12, entries);
      EXPECT_EQ( node2, TAILQ_NEXT(TAILQ_FIRST(&nodeHead), entries) );

      TAILQ_REMOVE(&nodeHead, node1, entries);
      EXPECT_EQ( node2, TAILQ_FIRST(&nodeHead) );

      TAILQ_REMOVE(&nodeHead, node2, entries);
      EXPECT_EQ( node4, TAILQ_FIRST(&nodeHead) );

      TAILQ_REMOVE(&nodeHead, node4, entries);
      EXPECT_EQ( node3, TAILQ_FIRST(&nodeHead) );
...
TEST(sys_queue, single_tail_queue)
{
  ...
  { SCOPED_TRACE("check init state");
    ASSERT_NE( STAILQ_EMPTY(&nodeHead), 0 );
    ASSERT_EQ( NULL, STAILQ_FIRST(&nodeHead) );
  }
  ...
  { SCOPED_TRACE("check relations");
    EXPECT_EQ( node1, STAILQ_FIRST(&nodeHead) );
    EXPECT_EQ( node12, STAILQ_NEXT(node1, entries) );
    EXPECT_EQ( NULL, STAILQ_NEXT(node3, entries) );
    EXPECT_EQ( node12, STAILQ_NEXT(STAILQ_FIRST(&nodeHead), entries) );
  }

  { SCOPED_TRACE("remove nodes");
    STAILQ_REMOVE_HEAD(&nodeHead, entries);
    EXPECT_EQ( node12, STAILQ_FIRST(&nodeHead) );

    STAILQ_REMOVE(&nodeHead, node12, stqEntry, entries);
    EXPECT_EQ( node2, STAILQ_FIRST(&nodeHead) );

    STAILQ_REMOVE(&nodeHead, node2, stqEntry, entries);
    EXPECT_EQ( node4, STAILQ_FIRST(&nodeHead) );

    STAILQ_REMOVE(&nodeHead, node4, stqEntry, entries);
    EXPECT_EQ( node3, STAILQ_FIRST(&nodeHead) );
...
TEST(sys_queue, simple_queue)
{
  ...
  { SCOPED_TRACE("check init state");
    ASSERT_NE( SIMPLEQ_EMPTY(&nodeHead), 0 );
    ASSERT_EQ( NULL, SIMPLEQ_FIRST(&nodeHead) );
  }
  ...
  { SCOPED_TRACE("check relations");
    EXPECT_EQ( node1, SIMPLEQ_FIRST(&nodeHead) );
    EXPECT_EQ( node12, SIMPLEQ_NEXT(node1, entries) );
    EXPECT_EQ( NULL, SIMPLEQ_NEXT(node3, entries) );
    EXPECT_EQ( node12, SIMPLEQ_NEXT(SIMPLEQ_FIRST(&nodeHead), entries) );
  }

  { SCOPED_TRACE("remove nodes");
    SIMPLEQ_REMOVE_HEAD(&nodeHead, entries);
    EXPECT_EQ( node12, SIMPLEQ_FIRST(&nodeHead) );

    SIMPLEQ_REMOVE(&nodeHead, node12, smplqEntry, entries);
    EXPECT_EQ( node2, SIMPLEQ_FIRST(&nodeHead) );

    SIMPLEQ_REMOVE(&nodeHead, node2, smplqEntry, entries);
    EXPECT_EQ( node4, SIMPLEQ_FIRST(&nodeHead) );

    SIMPLEQ_REMOVE(&nodeHead, node4, smplqEntry, entries);
    EXPECT_EQ( node3, SIMPLEQ_FIRST(&nodeHead) );
