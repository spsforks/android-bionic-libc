...
typedef struct {
  double expected;
  double call_data;
} significand_intel_data_t;

static significand_intel_data_t g_significand_intel_data[] = {
  { // Entry 0
    0x1.p0,
    0x1.0p100
  },
...
TEST(math_significand, significand_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_significand_intel_data)/sizeof(significand_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_significand_intel_data[i].expected, significand(g_significand_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
