...
typedef struct {
  double expected;
  double call_data;
} nearbyint_intel_data_t;

static nearbyint_intel_data_t g_nearbyint_intel_data[] = {
  { // Entry 0
    -0.0,
    -0x1.0p-1074
  },
...
TEST(math_nearbyint, nearbyint_intel) {
#if defined(__BIONIC__)
  fesetround(FE_TONEAREST);
  for (size_t i = 0; i < sizeof(g_nearbyint_intel_data)/sizeof(nearbyint_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_nearbyint_intel_data[i].expected, nearbyint(g_nearbyint_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
