...
static floor_intel_data_t g_floor_intel_data[] = {
  { // Entry 0
    -0x1.p0,
    -0x1.0p-1074
  },
...
TEST(math_floor, floor_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_floor_intel_data)/sizeof(floor_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_floor_intel_data[i].expected, floor(g_floor_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
