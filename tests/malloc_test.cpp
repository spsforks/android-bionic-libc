...
TEST(malloc, malloc_info) {
  ...
  if (version == "jemalloc-1") {
    ...
    // Only verify that this is debug-malloc-1 or scudo, the malloc debug unit tests
    // verify the output.
    ASSERT_TRUE(version == "scudo-1" || version == "debug-malloc-1") << "Unknown version: " << version;
  }
#endif
}
