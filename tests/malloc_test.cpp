...
TEST(malloc, verify_alignment) {
  ...
  for (size_t i = 0; i < MAX_LOOPS; i++) {
    ...
    *values_ldouble[i] = 5.5 + i;
    ...
    // On 32 bit devices, it's not necessary to have the alignment be the same
    // as the size of a long double.
#if defined(__LP64__)
    ASSERT_EQ(0U, reinterpret_cast<uintptr_t>(values_ldouble[i]) & 0xf);
#else
