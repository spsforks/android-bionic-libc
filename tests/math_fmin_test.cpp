...
static fmin_intel_data_t g_fmin_intel_data[] = {
  { // Entry 0
    -0x1.40p3,
    -0x1.4p3,
    -0x1.4p3
...
TEST(math_fmin, fmin_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_fmin_intel_data)/sizeof(fmin_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_fmin_intel_data[i].expected, fmin(g_fmin_intel_data[i].call_data1, g_fmin_intel_data[i].call_data2)) << "Failed on element " << i;
  }
#else // __BIONIC__
