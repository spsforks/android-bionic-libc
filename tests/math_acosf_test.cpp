...
static acosf_intel_data_t g_acosf_intel_data[] = {
  { // Entry 0
...
TEST(math_acosf, acosf_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_acosf_intel_data)/sizeof(acosf_intel_data_t); i++) {
    EXPECT_FLOAT_EQ(g_acosf_intel_data[i].expected, acosf(g_acosf_intel_data[i].call_data)) << "Failed on element " << i;
  }
#else // __BIONIC__
