...
static fdim_intel_data_t g_fdim_intel_data[] = {
  { // Entry 0
...
TEST(math_fdim, fdim_intel) {
#if defined(__BIONIC__)
  for (size_t i = 0; i < sizeof(g_fdim_intel_data)/sizeof(fdim_intel_data_t); i++) {
    EXPECT_DOUBLE_EQ(g_fdim_intel_data[i].expected, fdim(g_fdim_intel_data[i].call_data1, g_fdim_intel_data[i].call_data2)) << "Failed on element " << i;
  }
#else // __BIONIC__
