...
Android's dynamic linker has increasingly strict requirements for what
code it order to load. It is necessary that an application’s native
code follows the rules and recommendations in order to ensure a smooth
transition to recent Android releases.

Below we outline in detail important changes related to native code
...
uses them for sanity checking. Some developers try to strip them in an
...
no text relocations are necessary. Check the Gentoo documentation for
cookbook recipes.
