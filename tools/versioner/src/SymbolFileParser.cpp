...
constexpr int FUTURE_API_LEVEL = 10000;

std::unordered_map<std::string, Arch> ALL_ARCHES{
  {"arm", Arch::arm},
  {"arm64", Arch::arm64},
  {"x86", Arch::x86},
  {"x86_64", Arch::x86_64},
 ...
 private:
  ...
  std::optional<Version> parseVersion() {
    ...
    while (hasNextLine()) {
      ...
      if (android::base::StartsWith(curr_line, "extern \"C\" {")) {
        cpp_scope = true;
        continue;
      }
  ...
  bool isInArch(const TagList& tags) const {
    ...
    for (auto&& tag :  tags) {
      auto it = ALL_ARCHES.find(tag);
      if (it == ALL_ARCHES.end()) {
        continue;
      }
  ...
  bool isInApi(const TagList& tags) const {
    ...
    for (const std::string&tag : tags) {
      ...
      if (tag == "future") {
        return compilation_type.api_level == FUTURE_API_LEVEL;
      }
    }
