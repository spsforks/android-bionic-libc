...
struct HeaderLocationInformation {
  ...
  std::unordered_set<std::string> ignored_header_paths;
};
}
...
static CompilationRequirements collectRequirements(const Arch& arch,
  ...
  std::vector<std::string> headers =
      collectHeaders(location.header_dir, location.ignored_header_paths);
...
// FORTIFY headers aren't freestanding (they get included by other headers), so
// we pretend they don't exist.
static const char *const known_fortify_files[] = {
  "bits/fortify/fcntl.h",
  "bits/fortify/poll.h",
  "bits/fortify/socket.h",
  "bits/fortify/stat.h",
  "bits/fortify/stdio.h",
  "bits/fortify/stdlib.h",
  "bits/fortify/string.h",
  "bits/fortify/unistd.h",
};
...
int main(int argc, char** argv) {
  ...
  if (ignore_fortify_headers) {
    ...
    if (android::base::EndsWith(location.header_dir, "/")) {
      prefix = location.header_dir;
    } else {
      prefix = location.header_dir + "/";
    }
    for (const char* s : known_fortify_files) {
      location.ignored_header_paths.insert(prefix + s);
    }
  }
