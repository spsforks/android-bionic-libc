...
template <typename ELF>
static typename ELF::Phdr* FindOriginalFirstLoadSegment(typename ELF::Phdr* program_headers,
                                                        size_t count) {
  const typename ELF::Phdr* first_loadable_segment =
      FindFirstLoadSegment(program_headers, count);

  typename ELF::Phdr* original_first_loadable_segment = NULL;
...
template <typename ELF>
static Elf_Scn* FindSectionContainingHole(Elf* elf,
                                          typename ELF::Off hole_start,
                                          size_t hole_size) {
  Elf_Scn* section = NULL;
  ...
  while ((section = elf_nextscn(elf, section)) != NULL) {
    const typename ELF::Shdr* section_header = ELF::getshdr(section);
    ...
    // sections layout.
    typename ELF::Off offset = section_header->sh_offset;
    if (section_header->sh_offset >= hole_start) {
      offset -= hole_size;
    }
...
template <typename ELF>
static Elf_Scn* FindLastSectionInSegment(Elf* elf,
                                         typename ELF::Phdr* program_header,
                                         typename ELF::Off hole_start,
                                         ssize_t hole_size) {
  const typename ELF::Off segment_end =
...
template <typename ELF>
static void SortOrderSensitiveProgramHeaders(typename ELF::Phdr* program_headers,
                                             size_t count) {
  std::vector<typename ELF::Phdr*> orderable;
  ...
  for (size_t i = 1; i < orderable.size(); ++i) {
    for (size_t j = i; j > 0; --j) {
      typename ELF::Phdr* first = orderable[j - 1];
      typename ELF::Phdr* second = orderable[j];
...
// TODO (dimitry): Update this we no longer split segments but rather shift VirtAddr
...
void ElfFile<ELF>::ResizeSection(Elf* elf, Elf_Scn* section, size_t new_size,
  ...
  AdjustDynamicSectionForHole(dynamic_section, hole_size, relocations_type);
}
...
bool ElfFile<ELF>::PackTypedRelocations(std::vector<typename ELF::Rela>* relocations) {
  ...
  size_t data_padding_bytes = is_padding_relocations_ ? initial_bytes - packed_bytes_estimate : initial_bytes - hole_size - packed_bytes_estimate;
...
bool ElfFile<ELF>::UnpackTypedRelocations(const std::vector<uint8_t>& packed) {
      ...
      dynamic_base + data->d_size / sizeof(dynamics[0]));
  {
    ...
    dyn.d_un.d_ptr = section_header->sh_offset;
    ReplaceDynamicEntry<ELF>(relocations_type_ == REL ? DT_ANDROID_REL : DT_ANDROID_RELA, dyn, &dynamics);
  }
  {
    typename ELF::Dyn dyn;
    dyn.d_tag = relocations_type_ == REL ? DT_RELSZ : DT_RELASZ;
    dyn.d_un.d_val = section_header->sh_size;
    ReplaceDynamicEntry<ELF>(relocations_type_ == REL ? DT_ANDROID_RELSZ : DT_ANDROID_RELASZ, dyn, &dynamics);
  }
...
void ElfFile<ELF>::Flush() {
  ...
  LOG_IF(ERROR, file_bytes == -1) << "elf_update failed: " << elf_errmsg(elf_errno());
  CHECK(file_bytes > 0);
