...
static bool RestoreLoadSegmentAlignment(typename ELF::Phdr* program_headers,
  ...
  //   - a packable ELF executable has more than one LOAD segments;
  //   - before packing all LOAD segments have the same p_align;
  //   - on packing we reduce only one LOAD segments' p_align.
  if (program_header->p_align == kPageSize) {
    ...
    LOG(FATAL) << "Cannot find a LOAD segment from which to restore p_align";
  }
  return false;
}
