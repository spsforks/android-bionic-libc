...
static void RunUnpackRelocationsTestFor(const std::string& arch) {
  ...
  if (relocs_so != NULL && packed_relocs_so != NULL) {
    ...
    ASSERT_EQ(0, fseek(relocs_so, EI_CLASS, SEEK_SET)) << "Invalid file length: " << strerror(errno);
    uint8_t elf_class = 0;
    ASSERT_EQ(1, fread(&elf_class, 1, 1, relocs_so));
...
static void RunPackRelocationsTestFor(const std::string& arch) {
  ...
  if (relocs_so != NULL && packed_relocs_so != NULL) {
    // lets detect elf class
    ASSERT_EQ(0, fseek(packed_relocs_so, EI_CLASS, SEEK_SET)) << "Invalid file length: " << strerror(errno);
    uint8_t elf_class = 0;
    ASSERT_EQ(1, fread(&elf_class, 1, 1, packed_relocs_so));
