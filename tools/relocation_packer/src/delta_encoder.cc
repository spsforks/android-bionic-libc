...
void RelocationDeltaCodec<ELF>::Encode(const std::vector<ElfRela>& relocations,
  ...
  for (size_t group_start = 0; group_start < relocations.size(); ) {
    ...
    if (is_relocation_group_has_addend(group_flags) && is_relocation_grouped_by_addend(group_flags)) {
...
template <typename ELF>
void RelocationDeltaCodec<ELF>::DetectGroupFields(const ElfRela& reloc_one, const ElfRela& reloc_two,
