...
void RelocationDeltaCodec<R>::Encode(const std::vector<R>& relocations,
  ...
  ElfAddr start_offset = relocations.size() > 1 ? relocations[0].r_offset - (relocations[1].r_offset - relocations[0].r_offset) : relocations[0].r_offset;
  ...
  for (size_t group_start = 0; group_start < relocations.size(); ) {
    ...
    DetectGroup(relocations, group_start, previous_offset, &group_size, &group_flags, &group_offset_delta, &group_info, &group_addend);
