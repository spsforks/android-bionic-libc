...
void __dump_memcpy() {
  ...
  // There is still a race condition that could happen if the alarm occurs,
  // and then the program exits at the same time. This isn't likely to happen
  // so ignore it.
  atomic_store(&g_index, 0);
}
