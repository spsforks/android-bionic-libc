...
const sock_filter arm_system_filter[] = {
    ...
    BPF_JUMP(
        BPF_JMP | BPF_JGE | BPF_K, 164, 72,
        71),  // mlock|munlock|mlockall|munlockall|sched_setparam|sched_getparam|sched_setscheduler|sched_getscheduler|sched_yield|sched_get_priority_max|sched_get_priority_min|sched_rr_get_interval|nanosleep|mremap
    BPF_JUMP(
        BPF_JMP | BPF_JGE | BPF_K, 182, 71,
        70),  // prctl|rt_sigreturn|rt_sigaction|rt_sigprocmask|rt_sigpending|rt_sigtimedwait|rt_sigqueueinfo|rt_sigsuspend|pread64|pwrite64
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 190, 1, 0),
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 188, 69, 68),  // getcwd|capget|capset|sigaltstack|sendfile
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 196, 68,
             67),  // vfork|ugetrlimit|mmap2|truncate64|ftruncate64|stat64
    ...
    BPF_JUMP(
        BPF_JMP | BPF_JGE | BPF_K, 212, 60,
        59),  // getuid32|getgid32|geteuid32|getegid32|setreuid32|setregid32|getgroups32|setgroups32|fchown32|setresuid32|getresuid32|setresgid32|getresgid32
    ...
    BPF_JUMP(
        BPF_JMP | BPF_JGE | BPF_K, 240, 53,
        52),  // gettid|readahead|setxattr|lsetxattr|fsetxattr|getxattr|lgetxattr|fgetxattr|listxattr|llistxattr|flistxattr|removexattr|lremovexattr|fremovexattr|tkill|sendfile64
    BPF_JUMP(
        BPF_JMP | BPF_JGE | BPF_K, 249, 52,
        51),  // sched_setaffinity|sched_getaffinity|io_setup|io_destroy|io_getevents|io_submit|io_cancel|exit_group
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 256, 1, 0),
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 252, 50, 49),  // epoll_ctl
    BPF_JUMP(
        BPF_JMP | BPF_JGE | BPF_K, 269, 49,
        48),  // set_tid_address|timer_create|timer_settime|timer_gettime|timer_getoverrun|timer_delete|clock_settime|clock_gettime|clock_getres|clock_nanosleep|statfs64|fstatfs64|tgkill
    ...
    BPF_JUMP(
        BPF_JMP | BPF_JGE | BPF_K, 367, 23,
        22),  // fallocate|timerfd_settime|timerfd_gettime|signalfd4|eventfd2|epoll_create1|dup3|pipe2|inotify_init1|preadv|pwritev|rt_tgsigqueueinfo|perf_event_open|recvmmsg|accept4
    ...
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 378, 19,
             18),  // clock_adjtime|syncfs|sendmmsg|setns|process_vm_readv|process_vm_writev
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 389, 1, 0),
    BPF_JUMP(
        BPF_JMP | BPF_JGE | BPF_K, 388, 17,
        16),  // sched_setattr|sched_getattr|renameat2|seccomp|getrandom|memfd_create|bpf|execveat
