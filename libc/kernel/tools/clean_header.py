...
def print_error(noUpdate, msg):
    if noUpdate:
        panic(msg)
    sys.stderr.write("warning: " + msg)


def cleanupFile(dst_dir, src_dir, rel_path, noUpdate = True):
    ...
    if not os.path.exists(full_path):
        print_error(noUpdate, "file does not exist: '%s'\n" % full_path)
        return None, None

    if not os.path.isfile(full_path):
        print_error(noUpdate, "path is not a file: '%s'\n" % full_path)
        return None, None
    ...
    if not parser.parsed:
        sys.stderr.write( "error: can't parse '%s'" % full_path )
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    for path in args:
        dst_path, newdata = cleanupFile(dst_dir, src_dir, path, noUpdate)
        if not dst_path:
            continue
