...
# It uses libclang to parse the tokens. libclang can be downloaded at
# http://llvm.org/releases/download.html and installed with LLVM.
# This tool requires two environment variables properly set for libclang. The
# first one is PYTHONPATH to include the path for clang python bindings, which
# resides under </path/to/clang/source>/bindings/python. The other is
# LD_LIBRARY_PATH to include the path for runtime library libclang.so.
import clang.cindex
...
class Token(clang.cindex.Token):
    """class Token inherits the one in clang, with extra id property to hold
    ...
    @property
    def cursor(self):
        """overrides the cursor in clang Token"""
        return self._cursor
...
# Bug filed with upstream: http://llvm.org/bugs/show_bug.cgi?id=22243
def SourceRange__contains__(self, other):
    """determines if a given location is inside the range"""
...
class CppTokenizer:
    ...
    def getTokensWithCursors(self):
        """Helper method to return all tokens in an extent.

           The cursor property in a clang token doesn't provide enough
           information. Because it is queried based on single token without
           any context, i.e. via calling conf.lib.clang_annotateTokens() with
           only one token given. So we often see 'INVALID_FILE' in one token's
           cursor. In this function it passes all the available tokens to
           get more informative cursors."""
        tokens_memory = ctypes.POINTER(clang.cindex.Token)()
        tokens_count = ctypes.c_uint()
        extent = self.tu.cursor.extent

        conf.lib.clang_tokenize(self.tu, extent, ctypes.byref(tokens_memory),
        ...
        conf.lib.clang_annotateTokens(self.tu, tokens_memory, count, cursors_memory)

        tokens_array = ctypes.cast(tokens_memory, ctypes.POINTER(clang.cindex.Token * count)).contents
        token_group = TokenGroup(self.tu, tokens_memory, tokens_count)
    ...
    def parse(self):
        self.tokens = self.getTokensWithCursors()
...
def test_CppTokenizer():
    ...
    tester.expectList(["#", "an", "/", "example", tokLOGICAND, tokLPAREN, "01923_xy", tokRPAREN])

    tester.setTokenizer(CppLineTokenizer("FOO(BAR) && defined(BAZ)"))
    tester.expectList(["FOO", tokLPAREN, "BAR", tokRPAREN, tokLOGICAND, "defined", tokLPAREN, "BAZ", tokRPAREN])

    tester.setTokenizer(CppLineTokenizer(["/*", "#", "*/"]))
    tester.expectList([])
...
class CppExpr:
    ...
    def parsePrimary(self):
        ...
        # Checking for 'defined' needs to come first now because 'defined' is recognized as
        # IDENTIFIER.
...
class Block:
    ...
    def format_blocks(self, tokens, indent=0):
        """returns the formatted lines of strings with proper indentation."""
        ...
        while i < len(tokens):
            ...
            if t.id == '{':
                ...
                if i+1 < len(tokens) and (tokens[i+1].id == ';' or
                        tokens[i+1].id in set(['else', '__attribute__', '__attribute', '__packed']) or
...
class BlockList:
    ...
    def removeVarsAndFuncs(self, knownStatics=set()):
        ...
        # '}' as the function end. Fortunately we don't have such cases in
    ...
    def getBlocks(self, tokzer=None):
        ...
        def consume_extent(i, tokens, extent=None, detect_change=False):
            ...
            while i < len(tokens) and tokens[i].location in extent:
                if debugBlockParser:
                    print ' ' * 2, tokens[i].id, tokens[i].kind, tokens[i].cursor.kind
                if tokens[i].kind == TokenKind.COMMENT:
                    i += 1
                    continue
                if (detect_change and tokens[i].cursor.extent != extent and 
                    tokens[i].cursor.kind == CursorKind.PREPROCESSING_DIRECTIVE):
                    break
        ...
        while i < len(tokens):
            ...
            if debugBlockParser:
                print "%d: Processing [%s], kind=[%s], cursor=[%s], extent=[%s]" \
                        % (t.location.line, t.spelling, t.kind,
                           cursor.kind, self.short_extent(cursor.extent))
            ...
            if cursor.kind == CursorKind.PREPROCESSING_DIRECTIVE:
                ...
                j = i
                while j+1 < len(tokens) and tokens[j+1].kind == TokenKind.COMMENT:
                    j += 1
                if j+1 >= len(tokens):
                ...
                if directive == 'define':
                    ...
                    if (i+1 < len(tokens) and tokens[i+1].id == '(' and
                            tokens[i].location.column + len(tokens[i].spelling) == tokens[i+1].location.column):
                        while i < len(tokens):
                            id += tokens[i].id
                    ...
                    blocks.append(Block(ret, directive=directive, lineno=t.location.line, identifier=id))
        ...
        # parsed=True indicates a successful parsing, although may result an empty BlockList.
        self.parsed = True
...
def test_BlockParser():
    ...
    test_block_parsing(\
        ["foo", "  #  /* ahah */ if defined(__KERNEL__) /* more */", "bar", "#endif"],
