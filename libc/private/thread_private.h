...
#define _THREAD_PRIVATE_MUTEX_LOCK(name) pthread_mutex_lock(&__THREAD_NAME(name)._private_lock)
#define _THREAD_PRIVATE_MUTEX_UNLOCK(name) pthread_mutex_unlock(&__THREAD_NAME(name)._private_lock)

/* Note that these aren't compatible with the usual OpenBSD ones which
 * lazy-initialize! */
