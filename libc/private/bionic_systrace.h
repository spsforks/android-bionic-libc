...
namespace android {
  class BionicScopedTrace {
      ...
      BionicScopedTrace(const char* name) {
        ...
        if(atrace_marker_fd == -1) {
          ...
          if (ATRACE_TAG_BIONIC & tags) {
             ...
             * 
             * So the other thing I could do is just to still use ATRACE_MESSAGE_LENGTH,
             * but make it smaller.  I got its current size from the other implementation
             * of this tracing.
             */
    ...
    private:
      ...
      uint64_t tags = ATRACE_TAG_LAST + 1;
  };
};
