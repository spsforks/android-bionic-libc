...
class WPVectorHeader {
  size_t mElementCount;  // in elements
  size_t mMappingSize;   // in bytes
  ssize_t mMutatorCount;
  ssize_t padding;
};
...
class WPVector {
 ...
 public:
  ...
  ~WPVector() {
    if (header) {
      munmap(const_cast<WPVectorHeader*>(header), header->mMappingSize);
      header = nullptr;
    }
  }
...
class WPVectorMutator {
 ...
 public:
  ...
  explicit WPVectorMutator(WPVector<T>& vector)
    ...
    if (!header) {
      reserve(1);
    } else if (header->mMutatorCount == 0) {
      if (mprotect(header, header->mMappingSize, PROT_READ | PROT_WRITE) != 0) {
        __libc_fatal("WPVectorMutator failed to make mapping writable: %s",
                     strerror(errno));
      }
    }

    ++header->mMutatorCount;
  }
  ...
  ~WPVectorMutator() {
    if (header && --header->mMutatorCount == 0) {
      if (mprotect(header, header->mMappingSize, PROT_READ)) {
 ...
 public:
  ...
  void reserve(size_t count) {
    ...
    if (!header) {
      ...
      new_header->mElementCount = 0;
      new_header->mMappingSize = needed;
      new_header->mMutatorCount = 0;
      new_header->padding = -1;
      header = new_header;
    } else if (needed > header->mMappingSize) {
      void* result =
        mremap(header, header->mMappingSize, needed, MREMAP_MAYMOVE);
  ...
  void resize(size_t new_size) {
    size_t byte_count;
    if (__size_mul_overflow(new_size, sizeof(T), &byte_count)) {
      __libc_fatal("WPVectorMutator::resize(%zu) overflowed", new_size);
    }
    if (byte_count > PTRDIFF_MAX) {
      __libc_fatal(
        "WPVectorMutator resized to longer than PTRDIFF_MAX bytes (%zu)",
        byte_count);
    }
