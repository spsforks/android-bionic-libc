...
bool ThreadCaptureImpl::ListThreads(TidList& tids) {
  ...
  do {
    ...
    if (nread < 0) {
      log_error("failed to get directory entries from %s: %s", path, strerror(errno));
      return false;
    } else if (nread > 0) {
      ssize_t off = 0;
      while (off < nread) {
...
void ThreadCaptureImpl::PtraceDetach(pid_t tid, unsigned int signal) {
  ...
  if (ptrace(PTRACE_DETACH, tid, NULL, sig_ptr) < 0 && errno != ESRCH) {
    log_error("failed to detach from thread %d of process %d: %s", tid, pid_,
        strerror(errno));
  }
}
...
int ThreadCaptureImpl::PtraceAttach(pid_t tid) {
  ...
  if (ret < 0) {
    log_error("failed to attach to thread %d of process %d: %s", tid, pid_,
        strerror(errno));
    return -1;
  }
  ...
  if (ptrace(PTRACE_INTERRUPT, tid, 0, 0) < 0) {
    if (errno == ESRCH) {
      ...
      log_error("failed to interrupt thread %d of process %d: %s", tid, pid_,
          strerror(errno));
      PtraceDetach(tid, 0);
      return -1;
    }
...
bool ThreadCaptureImpl::PtraceThreadInfo(pid_t tid, ThreadInfo& thread_info) {
  ...
  if (ptrace(PTRACE_GETREGSET, tid, reinterpret_cast<void*>(NT_PRSTATUS), &iovec)) {
    log_error("ptrace getregset for thread %d of process %d failed: %s",
...
int ThreadCaptureImpl::CaptureThread(pid_t tid) {
  ...
  if (TEMP_FAILURE_RETRY(waitpid(tid, &status, __WALL)) < 0) {
    log_error("failed to wait for pause of thread %d of process %d: %s", tid, pid_,
        strerror(errno));
    PtraceDetach(tid, 0);
    return -1;
  }
  ...
  if (!WIFSTOPPED(status)) {
    log_error("thread %d of process %d was not paused after waitpid, killed?",
        tid, pid_);
    return 0;
  }
  ...
  if ((status >> 16) == PTRACE_EVENT_STOP) {
    switch (signal) {
      ...
      case SIGTRAP:
        ...
        break;
      default:
        log_error("unexpected signal %d with PTRACE_EVENT_STOP for thread %d of process %d",
            signal, tid, pid_);
        return -1;
    }
  } else {
