...
class Memleak {
 public:
  ...
  size_t DetectLeaks(allocator::vector<Leak>& leaks, size_t limit);
...
size_t Memleak::DetectLeaks(allocator::vector<Leak>& leaks, size_t limit) {
  log_info("sweeping process %d for memory leaks", pid_);
  leaks.clear();

  allocator::vector<Range> leaked{allocator_};
  size_t num_leaks = heap_walker_.Leaked(leaked, limit);
...
bool DetectLeaks(std::vector<Leak>& leaks, size_t limit) {
  ...
  PtracerThread thread{[&]() -> int {
    ...
    if (ret < 0) {
      ...
      size_t num_leaks = memleak.DetectLeaks(leaks, limit);
  ...
  log_error("memory leak detection done, %zu leaks found out of %zu allocations",
      num_leaks, num_allocations);
  return true;
}
...
static void LogLeak(Leak& leak, bool log_contents) {
  ...
  if (log_contents) {
   ...
   for (size_t i = 0; i < bytes; i += bytes_per_line) {
     ptr = buf;
     ptr += snprintf(ptr, sizeof(buf) - (ptr - buf), "%" PRIxPTR ": ", leak.begin + i);
     size_t j;
     for (j = i; j < bytes && j < i + bytes_per_line; j++) {
       ptr += snprintf(ptr, sizeof(buf) - (ptr - buf), "%02x ", leak.contents[j]);
     }
     ...
     for (j = i; j < bytes && j < i + bytes_per_line; j++) {
       ...
       ptr += snprintf(ptr, sizeof(buf) - (ptr - buf), "%c", c);
     }
     ptr += snprintf(ptr, sizeof(buf) - (ptr - buf), "\n");
     log_error("%s", buf);
   }
