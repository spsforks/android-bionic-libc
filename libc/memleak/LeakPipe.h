...
#ifndef LIBMEMLEAK_LEAK_PIPE_H_
#define LIBMEMLEAK_LEAK_PIPE_H_
...
class LeakPipe {
 public:
  LeakPipe() {
    ...
    if (ret < 0) {
      log_fatal("failed to create socketpair: %s", strerror(errno));
    }
  }
  ...
  class LeakPipeSender : public LeakPipeBase {
   public:
    ...
    bool Send(const T& value) {
      ...
      if (ret < 0) {
        log_error("failed to send value: %s", strerror(errno));
        return false;
      } else if (static_cast<size_t>(ret) != sizeof(T)) {
        log_error("eof while writing value");
        return false;
      }
    ...
    bool SendVector(const std::vector<T, Alloc>& vector) {
      ...
      if (ret < 0) {
        log_error("failed to send vector: %s", strerror(errno));
        return false;
      } else if (static_cast<size_t>(ret) != size) {
        log_error("eof while writing vector");
        return false;
      }
  ...
  class LeakPipeReceiver : public LeakPipeBase {
   public:
    ...
    bool Receive(T* value) {
      ...
      if (ret < 0) {
        log_error("failed to receive value: %s", strerror(errno));
        return false;
      } else if (static_cast<size_t>(ret) != sizeof(T)) {
        log_error("eof while receiving value");
        return false;
      }
    ...
    bool ReceiveVector(std::vector<T, Alloc>& vector) {
      ...
      while (size > 0) {
        ...
        if (ret < 0) {
          ...
          log_error("eof while reading vector");
          return false;
        }
        size -= ret;
        ptr += ret;
