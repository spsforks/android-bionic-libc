...
/*
 * The following two functions are identical, but were added in a later
 * API level (21 rather than 19). Since they're identical, we can actually
 * just use __RENAME, in which case the resulting object files will work
 * fine on API level 19 devices.
 */

/** Equivalent to statvfs(). */
int statvfs64(const char* __path, struct statvfs64* __buf) __RENAME(statvfs) __INTRODUCED_IN(19);

/** Equivalent to fstatvfs(). */
int fstatvfs64(int __fd, struct statvfs64* __buf) __RENAME(fstatvfs) __INTRODUCED_IN(19);
