...
// __BIONIC_INTERNAL__ is used to guard the following macros for use within
// bionic only. If this header file gets included in some other package
// that is NOT bionic, then it must NOT take effect!
...
#if defined(LIBC_INCLUDE_MALLOC_H_) ||  \
    defined(LIBC_BIONIC_DLMALLOC_H_) || \
    defined(LIBC_BIONIC_JEMALLOC_H_)

// Because of the rather complex ways in which malloc and friends are
// implemented in Bionic, the macro replacements --
// e.g. calls to malloc() are swizzled out to be calls to __bionic_malloc()
// -- are also somewhat more convoluted, by necessity.

// jemalloc is the most problematic. Due to use of __attribute__((malloc)), we
// need to delay the definition of the macros belows until AFTER the rest of
// the jemalloc headers are included. Otherwise we windup with
// __attribute__((__bionic_malloc)) which is nonsensical.
...
#if !defined(DELAY_JEMALLOC_REDEFINE)
