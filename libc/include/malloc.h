 ...
 * deterministic detection linear buffer overflow and underflow bugs
 * by assigning distinct tag values to adjacent allocations. This mode
 * has a slightly reduced chance to detect use-after-free bugs because
 * only half of the possible tag values are available for each memory
 * location.
 *
