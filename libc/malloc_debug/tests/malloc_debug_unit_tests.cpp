...
TEST_F(MallocDebugTest, front_guard_corrupted) {
  ...
  backtrace_fake_add(0, 0x0001);
  backtrace_fake_add(0, 0x0002);
  backtrace_fake_add(0, 0x0003);
...
TEST_F(MallocDebugTest, rear_guard_corrupted) {
  ...
  backtrace_fake_add(0, 0x0100);
  backtrace_fake_add(0, 0x0200);
  backtrace_fake_add(0, 0x0300);
...
TEST_F(MallocDebugTest, tag_corrupted) {
  ...
  backtrace_fake_add(0, 0xa);
  backtrace_fake_add(0, 0xb);
  backtrace_fake_add(0, 0xc);

  backtrace_fake_add(1, 0xaa);
  backtrace_fake_add(1, 0xbb);
  backtrace_fake_add(1, 0xcc);

  backtrace_fake_add(2, 0xaaa);
  backtrace_fake_add(2, 0xbbb);
  backtrace_fake_add(2, 0xccc);
...
TEST_F(MallocDebugTest, leak_track_no_frees_with_backtrace) {
  ...
  backtrace_fake_add(0, 0x1000);
  backtrace_fake_add(0, 0x2000);
  backtrace_fake_add(0, 0x3000);

  void* pointer1 = debug_malloc(100);
  ASSERT_TRUE(pointer1 != nullptr);
  memset(pointer1, 0, 100);

  backtrace_fake_add(1, 0xa000);
  backtrace_fake_add(1, 0xb000);
  backtrace_fake_add(1, 0xc000);
  backtrace_fake_add(1, 0xd000);

  void* pointer2 = debug_malloc(128);
  ASSERT_TRUE(pointer2 != nullptr);
  memset(pointer2, 0, 128);

  backtrace_fake_add(2, 0xfe000);
  backtrace_fake_add(2, 0xde000);
  backtrace_fake_add(2, 0xce000);
  backtrace_fake_add(2, 0xbe000);
  backtrace_fake_add(2, 0xae000);
...
TEST_F(MallocDebugTest, leak_track_frees) {
  ...
  ASSERT_STREQ("", getFakeLogBuf().c_str());
  std::string expected_log = android::base::StringPrintf(
      "6 malloc_debug +++ malloc_testing leaked block of size 100 at %p (leak 1 of 1)\n",
      pointer1);
  ASSERT_STREQ(expected_log.c_str(), getFakeLogPrint().c_str());
...
TEST_F(MallocDebugTest, leak_track_frees_finalize) {
  Init("leak_track");
  ...
  void* pointer2 = debug_malloc(100);
  ASSERT_TRUE(pointer2 != nullptr);
  memset(pointer2, 0, 100);
  ...
  ASSERT_STREQ("", getFakeLogBuf().c_str());
  std::string expected_log = android::base::StringPrintf(
      "6 malloc_debug +++ malloc_testing leaked block of size 100 at %p (leak 1 of 2)\n",
      pointer2);
  expected_log += android::base::StringPrintf(
      "6 malloc_debug +++ malloc_testing leaked block of size 390 at %p (leak 2 of 2)\n",
      pointer1);
  ASSERT_STREQ(expected_log.c_str(), getFakeLogPrint().c_str());
}
...
TEST_F(MallocDebugTest, free_track_use_after_free_with_backtrace) {
  ...
  // Alloc backtrace.
  backtrace_fake_add(0, 0xf);
  backtrace_fake_add(0, 0xe);
  backtrace_fake_add(0, 0xd);
  // Free backtrace.
  backtrace_fake_add(1, 0xfa);
  backtrace_fake_add(1, 0xeb);
  backtrace_fake_add(1, 0xdc);
...
TEST_F(MallocDebugTest, get_malloc_leak_info_single) {
  ...
  size_t individual_size = 2 * sizeof(size_t) + 32 * sizeof(uintptr_t);
  ...
  for (size_t i = 0; i < entry->num_frames; i++) {
    backtrace_fake_add(0, entry->frames[i]);
  }
...
TEST_F(MallocDebugTest, get_malloc_leak_info_multi) {
  ...
  for (size_t i = 0; i < entry2->num_frames; i++) {
    backtrace_fake_add(0, entry2->frames[i]);
  }
  ...
  for (size_t i = 0; i < 16; i++) {
    entry1->frames[0] = 0xbc000 + i;
  }

  for (size_t i = 0; i < entry1->num_frames; i++) {
    backtrace_fake_add(1, entry1->frames[i]);
  }
  // This entry should not be in the frames since it would go over the total
  // number of frames.
  backtrace_fake_add(1, 0xffff);
...
TEST_F(MallocDebugTest, realloc_usable_size) {
  ...
  // copies the bytes in the usable size.
