...
that only allocations of a size greater than 'ALLOCATION\_SIZE\_BYTES'
will be backtraced. When used in combination with the 'backtrace\_max'
option, then allocations >= than 'backtrace\_min\_size' and <=
...
This option can also be used in combination with another tools such
...
As of U, setting this in combination with the backtrace option means
that only allocations of a size less than 'ALLOCATION\_SIZE\_BYTES'
will be backtraced. When used in combination with the 'backtrace\_min'
option, then allocations >= than 'backtrace\_min\_size' and <=
'backtrace\_max\_size' will be backtraced. The 'backtrace\_size' option
overrides this option, and should not be used at the same time.
...
This can be used to help performance by only targetting allocations of
a certain size.
