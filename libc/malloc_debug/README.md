...
used with native servers or apps that run for a while since these processes
are not spawned from a zygote process.

Note that when the signal is received, the heap is not dumped to a file until
the next malloc/free occurs.
