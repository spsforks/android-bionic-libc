...
{
    if (DBG) {
        __safe_format_log(ANDROID_LOG_INFO, "libc", "rcode = %d, sec = %d", rcode, rtt);
    }
    sample->at = now;
    sample->rcode = rcode;
    sample->rtt = rtt;
...
_res_stats_usable_server(const struct __res_params* params, struct __res_stats* stats) {
    ...
    if (successes >= 0 && errors >= 0 && timeouts >= 0) {
        ...
        if (DBG) {
            __safe_format_log(ANDROID_LOG_DEBUG, "libc", "NS stats: S %d + E %d + T %d + I %d "
        ...
        if (total >= params->min_samples && (errors > 0 || timeouts > 0)) {
            ...
            if (DBG) {
                __safe_format_log(ANDROID_LOG_DEBUG, "libc", "success rate %d%%\n", success_rate);
            }
            if (success_rate < params->success_threshold) {
                ...
                if (now - last_sample_time > params->sample_validity) {
                    ...
                    if (DBG) {
                        __safe_format_log(ANDROID_LOG_INFO, "libc",
