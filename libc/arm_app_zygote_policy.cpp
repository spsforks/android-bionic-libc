...
const sock_filter arm_app_zygote_filter[] = {
    ...
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 7, 132,
             131),  // restart_syscall|exit|fork|read|write|open|close
    ...
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 149, 78,
             77),  //_llseek|getdents|_newselect|flock|msync|readv|writev|getsid|fdatasync
    BPF_JUMP(
        BPF_JMP | BPF_JGE | BPF_K, 164, 77,
        76),  // mlock|munlock|mlockall|munlockall|sched_setparam|sched_getparam|sched_setscheduler|sched_getscheduler|sched_yield|sched_get_priority_max|sched_get_priority_min|sched_rr_get_interval|nanosleep|mremap
    ...
    BPF_JUMP(
        BPF_JMP | BPF_JGE | BPF_K, 182, 73,
        72),  // prctl|rt_sigreturn|rt_sigaction|rt_sigprocmask|rt_sigpending|rt_sigtimedwait|rt_sigqueueinfo|rt_sigsuspend|pread64|pwrite64
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 190, 1, 0),
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 188, 71, 70),  // getcwd|capget|capset|sigaltstack|sendfile
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 198, 70,
             69),  // vfork|ugetrlimit|mmap2|truncate64|ftruncate64|stat64|lstat64|fstat64
    ...
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 212, 61,
             60),  // fchown32|setresuid32|getresuid32|setresgid32|getresgid32
    ...
    BPF_JUMP(
        BPF_JMP | BPF_JGE | BPF_K, 240, 55,
        54),  // gettid|readahead|setxattr|lsetxattr|fsetxattr|getxattr|lgetxattr|fgetxattr|listxattr|llistxattr|flistxattr|removexattr|lremovexattr|fremovexattr|tkill|sendfile64
    BPF_JUMP(
        BPF_JMP | BPF_JGE | BPF_K, 249, 54,
        53),  // sched_setaffinity|sched_getaffinity|io_setup|io_destroy|io_getevents|io_submit|io_cancel|exit_group
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 256, 1, 0),
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 254, 52,
             51),  // epoll_create|epoll_ctl|epoll_wait|remap_file_pages
    BPF_JUMP(
        BPF_JMP | BPF_JGE | BPF_K, 262, 51,
        50),  // set_tid_address|timer_create|timer_settime|timer_gettime|timer_getoverrun|timer_delete
    ...
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 269, 46,
             45),  // clock_gettime|clock_getres|clock_nanosleep|statfs64|fstatfs64|tgkill
    ...
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 298, 38,
             37),  // recvfrom|shutdown|setsockopt|getsockopt|sendmsg|recvmsg
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 319, 37,
             36),  // inotify_init|inotify_add_watch|inotify_rm_watch
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 327, 1, 0),
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 326, 35, 34),  // openat|mkdirat|mknodat|fchownat
    BPF_JUMP(
        BPF_JMP | BPF_JGE | BPF_K, 338, 34,
        33),  // fstatat64|unlinkat|renameat|linkat|symlinkat|readlinkat|fchmodat|faccessat|pselect6|ppoll|unshare
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 397, 17, 0),
    ...
    BPF_JUMP(
        BPF_JMP | BPF_JGE | BPF_K, 367, 24,
        23),  // timerfd_create|eventfd|fallocate|timerfd_settime|timerfd_gettime|signalfd4|eventfd2|epoll_create1|dup3|pipe2|inotify_init1|preadv|pwritev|rt_tgsigqueueinfo|perf_event_open|recvmmsg|accept4
    ...
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 378, 20,
             19),  // syncfs|sendmmsg|setns|process_vm_readv|process_vm_writev
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 386, 19,
             18),  // sched_setattr|sched_getattr|renameat2|seccomp|getrandom|memfd_create
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 389, 1, 0),
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 388, 17, 16),  // execveat
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 394, 16,
             15),  // membarrier|mlock2|copy_file_range|preadv2|pwritev2
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 434, 7, 0),
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 417, 3, 0),
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 403, 1, 0),
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 398, 12, 11),  // statx
    BPF_JUMP(
        BPF_JMP | BPF_JGE | BPF_K, 415, 11,
        10),  // clock_gettime64|clock_settime64|clock_adjtime64|clock_getres_time64|clock_nanosleep_time64|timer_gettime64|timer_settime64|timerfd_gettime64|timerfd_settime64|utimensat_time64|pselect6_time64|ppoll_time64
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 420, 1, 0),
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 418, 9, 8),  // recvmmsg_time64
    BPF_JUMP(
        BPF_JMP | BPF_JGE | BPF_K, 424, 8,
        7),  // semtimedop_time64|rt_sigtimedwait_time64|futex_time64|sched_rr_get_interval_time64
...
const size_t arm_app_zygote_filter_size =
    sizeof(arm_app_zygote_filter) / sizeof(struct sock_filter);
