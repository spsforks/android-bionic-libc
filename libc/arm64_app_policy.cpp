...
const sock_filter arm64_app_filter[] = {
    ...
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 163, 31, 0),
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 101, 15, 0),
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 43, 7, 0),
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 30, 3, 0),
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 19, 1, 0),
    BPF_JUMP(
        BPF_JMP | BPF_JGE | BPF_K, 18, 56,
        55),  // io_setup|io_destroy|io_submit|io_cancel|io_getevents|setxattr|lsetxattr|fsetxattr|getxattr|lgetxattr|fgetxattr|listxattr|llistxattr|flistxattr|removexattr|lremovexattr|fremovexattr|getcwd
    BPF_JUMP(
        BPF_JMP | BPF_JGE | BPF_K, 29, 55,
        54),  // eventfd2|epoll_create1|epoll_ctl|epoll_pwait|dup|dup3|fcntl|inotify_init1|inotify_add_watch|inotify_rm_watch
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 41, 1, 0),
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 39, 53,
             52),  // ioprio_set|ioprio_get|flock|mknodat|mkdirat|unlinkat|symlinkat|linkat|renameat
    ...
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 51, 49,
             48),  // statfs|fstatfs|truncate|ftruncate|fallocate|faccessat|chdir|fchdir
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 58, 48,
             47),  // fchmod|fchmodat|fchownat|fchown|openat|close
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 90, 1, 0),
    BPF_JUMP(
        BPF_JMP | BPF_JGE | BPF_K, 89, 46,
        45),  // pipe2|quotactl|getdents64|lseek|read|write|readv|writev|pread64|pwrite64|preadv|pwritev|sendfile|pselect6|ppoll|signalfd4|vmsplice|splice|tee|readlinkat|newfstatat|fstat|sync|fsync|fdatasync|sync_file_range|timerfd_create|timerfd_settime|timerfd_gettime|utimensat
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 98, 45,
             44),  // capget|capset|personality|exit|exit_group|waitid|set_tid_address|unshare
    ...
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 112, 40,
             39),  // timer_create|timer_gettime|timer_getoverrun|timer_settime|timer_delete
    ...
    BPF_JUMP(
        BPF_JMP | BPF_JGE | BPF_K, 142, 37,
        36),  // ptrace|sched_setparam|sched_setscheduler|sched_getscheduler|sched_getparam|sched_setaffinity|sched_getaffinity|sched_yield|sched_get_priority_max|sched_get_priority_min|sched_rr_get_interval|restart_syscall|kill|tkill|tgkill|sigaltstack|rt_sigsuspend|rt_sigaction|rt_sigprocmask|rt_sigpending|rt_sigtimedwait|rt_sigqueueinfo|rt_sigreturn|setpriority|getpriority
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 153, 3, 0),
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 150, 1, 0),
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 149, 34, 33),  // setresuid|getresuid
    BPF_JUMP(BPF_JMP | BPF_JGE | BPF_K, 151, 33, 32),  // getresgid
