...
# to be placed into arch-{arm,x86,x86_64,mips}/syscalls, as well as the content
# of arch-{arm,x86,x86_64,mips}/linux/_syscalls.h
#
...
x86_64_header = """/* autogenerated by gensyscalls.py */
#include <linux/err.h>
#include <machine/asm.h>
#include <asm/unistd.h>
...
class State:
        ...
        for t in self.syscalls:
            ...
            if t["common"] >= 0 or t["x86id"] >= 0:
                ...
                E("cid for dispatch syscalls is only supported for x86 in "
                  "'%s'" % syscall_name)
                return
            if t["common"] >= 0 or t["x86_64id"] >= 0:
                num_regs = count_generic_param_registers64(syscall_params)
                t["asm-x86_64"] = self.x86_64_genstub(syscall_func, num_regs, make__NR_name(syscall_name))
    ...
    def gen_glibc_syscalls_h(self):
        ...
        glibc_fp.write("#if defined(__ILP32__)\n")
        self.scan_linux_unistd_h(glibc_fp, bionic_libc_root + "/kernel/arch-x86/asm/unistd_x32.h")
        glibc_fp.write("#else\n")
        self.scan_linux_unistd_h(glibc_fp, bionic_libc_root + "/kernel/arch-x86/asm/unistd_64.h")
        glibc_fp.write("#endif /* __ILP32__ */\n")
    ...
    def gen_arch_syscalls_mk(self, arch):
        ...
        arch_test = {
            ...
            "x86_64": lambda x: x.has_key("asm-x86_64"),
    ...
    def gen_syscall_stubs(self):
        for sc in self.syscalls:
            ...
            if sc.has_key("asm-x86_64") and 'x86_64' in all_archs:
                fname = "arch-x86_64/syscalls/%s.S" % sc["func"]
                D2( ">>> generating "+fname )
                fp = create_file( fname )
                fp.write(sc["asm-x86_64"])
                fp.close()
                self.new_stubs.append( fname )
