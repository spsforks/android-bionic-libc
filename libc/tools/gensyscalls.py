...
class State:
    ...
    def gen_linux_syscalls_h(self):
        ...
        fp.write( "\n#ifdef __arm__\n" );
        for sc in sorted(self.syscalls,key=lambda x:x["armid"]):
            sc_id = sc["armid"]
            sc_name = sc["name"]
            if sc_id >= 0:
                self.gen_NR_syscall( fp, sc_name, sc_id )
        ...
        for sc in sorted(self.syscalls,key=lambda x:x["shid"]):
            sc_id = sc["shid"]
            sc_name = sc["name"]
            if sc_id >= 0:
                self.gen_NR_syscall( fp, sc_name, sc_id )
    ...
    def gen_syscall_stubs(self):
        for sc in self.syscalls:
            ...
            if sc.has_key("asm-sh") and 'sh' in all_archs:
