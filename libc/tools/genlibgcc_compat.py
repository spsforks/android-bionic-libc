...
class Generator:
    def process(self):
	    ...
	    print "* Build complete, logfile: " + build_output_file_path
            ...
            arch_variant = subprocess.check_output(["CALLED_FROM_SETUP=true BUILD_SYSTEM=build/core make --no-print-directory -f build/core/config.mk dumpvar-TARGET_ARCH_VARIANT"],
                    cwd=android_build_top_path, shell=True).strip()

            if arch_variant != 'silvermont':
                sys.exit("Error: for x86, only silvermont arch needed")
