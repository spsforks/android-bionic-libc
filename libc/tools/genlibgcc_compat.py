...
"""
#define   COMPAT_SYM_LIST \\
"""

libgcc_compat_footer = """
#define  XX(f)    extern void* f;
COMPAT_SYM_LIST
#undef XX

void* __bionic_libgcc_compat_symbols[COMPAT_SYM_CNT];

void __bionic_libgcc_compat_hooks(void) {
    int index = 0;
#define XX(f)   __bionic_libgcc_compat_symbols[index++] = f; 
COMPAT_SYM_LIST
#undef XX
}
"""
...
class Generator:
    def process(self):
        ...
        for sym_name in symbol_list:
            fres.write("    XX("+sym_name+") \\\n")
        fres.write("\n#define COMPAT_SYM_CNT " + repr(len(symbol_list)+1) + "\n")
