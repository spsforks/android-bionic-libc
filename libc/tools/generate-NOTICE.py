...
def is_interesting(path):
    path = path.lower()
    if path.endswith(".bp") or path.endswith(".mk") or path.endswith(".map") or path.endswith(".py") or path.endswith(".pyc") or path.endswith(".txt") or path.endswith(".3") or path.endswith(".swp"):
        return False
...
def do_file(path):
    ...
    for i, line in enumerate(lines):
        if "Copyright" in line and not "@(#) Copyright" in line:
