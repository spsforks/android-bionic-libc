...
def merge_names(base_names, whitelist_names, blacklist_names):
  ...
  if bool(blacklist_names & whitelist_names):
    raise RuntimeError("Blacklist item in whitelist - aborting " + str(
        blacklist_names & whitelist_names))
...
ANDROID_SYSTEM_SYSCALL_FILES = {
    "base": "SYSCALLS.TXT",
    "whitelists": ["SECCOMP_WHITELIST_SYSTEM.TXT"],
    "blacklists": ["SECCOMP_BLACKLIST_COMMON.TXT"]
}

ANDROID_APP_SYSCALL_FILES = {
    "base": "SYSCALLS.TXT",
    "whitelists": ["SECCOMP_WHITELIST_APP.TXT"],
    "blacklists": ["SECCOMP_BLACKLIST_COMMON.TXT"]
}
...
ANDROID_GLOBAL_SYSCALL_FILES = {
    ...
    "blacklists": ["SECCOMP_BLACKLIST_GLOBAL.TXT"]
}
