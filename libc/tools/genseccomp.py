...
def get_clang_path():
  ...
  # First try to see if clang-stable has a usable clang binary.
  ...
  # Otherwise, return the latest version. Note that the versions that start with
  # 'r' are newer, but those collate after the old versioning scheme of just
  # digits, so everything works out just fine.
  versioned_clang_path = os.path.join(prebuilts_root, "clang-*/bin/clang")
