...
def CreateOneFile(architecture, syscall_file, header_dir, output_file):
    ...
    # Run preprocessor over the __NR_syscall symbols, including unistd.h, to get the
    # actual numbers
    prefix = "__SECCOMP_" # prefix to (almost) ensure no name collisions
    cpp = Popen(['../../prebuilts/misc/linux-x86/analyzer/bin/clang',
    ...
    # Note that some of the numbers were expressed as base + offset, so we need to eval, not just int
...
CreateOneFile("arm", syscall_file, "kernel/uapi/asm-arm", "seccomp/arm_policy.c")
CreateOneFile("arm64", syscall_file, "kernel/uapi/asm-arm64", "seccomp/arm64_policy.c")
