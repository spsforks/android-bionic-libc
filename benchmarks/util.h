...
extern std::map<std::string, benchmark_func_t> str_to_func;

static int  __attribute__((unused)) emplace_benchmark (std::string fn_name, benchmark_func_t fn_ptr) {
  map_lock.lock();
  str_to_func.emplace(std::string(fn_name), fn_ptr);
  map_lock.unlock();
  return 0;
}

#define BIONIC_BENCHMARK(n) \
  int _bionic_benchmark_##n __attribute__((unused)) = emplace_benchmark(std::string(#n), reinterpret_cast<benchmark_func_t>(n))
