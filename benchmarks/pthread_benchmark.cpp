...
static void* spam_mmap_sem_thread(void*) {
  ...
  while (!spam_threads_exit_please) {
    void* mapping = mmap(nullptr, region_size, PROT_READ | PROT_WRITE,
...
static void BM_pthread_create_and_run_with_mmap_sem_contention(benchmark::State& state) {
  ...
  for (size_t i = 0; i < nr_spamming_threads; ++i) {
    if (pthread_create(&spamming_threads[i], nullptr,
                       spam_mmap_sem_thread, nullptr)) {
      abort();
    }
  }
  state.ResumeTiming();
  ...
  for (size_t i = 0; i < nr_spamming_threads; ++i) {
    void* retval;
    if (pthread_join(spamming_threads[i], &retval)) {
      abort();
    }
  }
  state.ResumeTiming();
