...
static void BM_string_memmove_non_overlapping(benchmark::State& state) {
  ...
  while (state.KeepRunning()) {
    memmove(&dst.front(), &src.front(), nbytes);
  }
...
static void BM_string_memmove_overlapping(benchmark::State& state) {
  const size_t nbytes = state.range(0);
  std::vector<char> buf(nbytes + 1, 'x');

  while (state.KeepRunning()) {
    memmove(&buf.front(), &buf.front() + 1, nbytes); // Worst-case overlap.
  }

  state.SetBytesProcessed(uint64_t(state.iterations()) * uint64_t(nbytes));
}
BENCHMARK(BM_string_memmove_overlapping)->AT_COMMON_SIZES;
