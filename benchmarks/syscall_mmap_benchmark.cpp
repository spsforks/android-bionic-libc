...
static void MmapFixedBenchmark(benchmark::State& state, const struct MmapParams& params, int fd,
  ...
  area = (uint8_t*)mmap(0, area_size, params.prot, params.flags, fd, 0);
  if (area == MAP_FAILED) {
...
static int PrepFile(benchmark::State& state, const char* path, size_t size) {
  int fd;
...
static void MmapFileBenchmark(benchmark::State& state, const struct MmapParams& params,
  ...
  fd = PrepFile(state, tf.path, area_size);
  if (fd < 0) {
    return;
  }

  if (params.flags & MAP_FIXED) {
    MmapFixedBenchmark(state, params, fd, area_size, offs);
  } else {
    MmapBenchmark(state, params, fd, NULL);
  }
...
static void BM_syscall_mmap_anon_rw(benchmark::State& state) {
  ...
  MmapBenchmark(state, params, 0, NULL);
}
...
BIONIC_BENCHMARK_WITH_ARG(BM_syscall_mmap_file_rw_priv_fixed_start, "AT_COMMON_SIZES");
...
// mapping at sub-page size offset is not supported, so run only for AT_LARGE_SIZES
BIONIC_BENCHMARK_WITH_ARG(BM_syscall_mmap_file_rw_priv_fixed_mid, "AT_LARGE_SIZES");
...
// mapping at sub-page size offset is not supported, so run only for AT_LARGE_SIZES
BIONIC_BENCHMARK_WITH_ARG(BM_syscall_mmap_file_rw_priv_fixed_end, "AT_LARGE_SIZES");
