...
static constexpr char kSizeStr[] = "_SIZE_";
static constexpr char kAlign2Str[] = "_ALIGN2_";
...
static bool ParseOnebufManualStr(std::string& arg, std::vector<int>* values) {
  ...
  //   XX is the alignment
  //   YY is the size
  char* start = &arg[sizeof(kOnebufManualStr) - 1];
  char* end;
  long int align = strtol(start, &end, 10);
  if (*end != '_' || start == end) {
    return false;
  }

  if (strncmp(end, kSizeStr, sizeof(kSizeStr) - 1) != 0) {
    return false;
  }
  start = &end[sizeof(kSizeStr) - 1];
  long int size = strtol(start, &end, 10);
  if (*end != '\0' || start == end) {
    return false;
  }
...
static bool ParseTwobufManualStr(std::string& arg, std::vector<int>* values) {
  ...
  //   ZZ is the size
  char* start = &arg[sizeof(kTwobufManualStr) - 1];
  char* end;
  long int align1 = strtol(start, &end, 10);
  if (*end != '_' || start == end) {
    return false;
  }

  if (strncmp(end, kAlign2Str, sizeof(kAlign2Str) - 1) != 0) {
    return false;
  }
  start = &end[sizeof(kAlign2Str) - 1];
  long int align2 = strtol(start, &end, 10);
  if (*end != '_' || start == end) {
    return false;
  }
