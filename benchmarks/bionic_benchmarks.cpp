...
void RegisterGoogleBenchmarks(bench_opts_t xml_opts, bench_opts_t cmdline_opts,
                         std::string fn_name, std::vector<std::vector<int>> *run_args,
                         const std::map<std::string, benchmark_func_t> &str_to_func) {
  if (!str_to_func.at(fn_name)) {
    printf("No benchmark for function %s\n", fn_name.c_str());
    abort();
  }
  int iterations_to_use = xml_opts.num_iterations ? xml_opts.num_iterations : \
                                                    cmdline_opts.num_iterations;
  int cpu_to_use = INT_MAX;
  if (xml_opts.cpu_to_lock != INT_MAX) {
    cpu_to_use = xml_opts.cpu_to_lock;

  } else if (cmdline_opts.cpu_to_lock != INT_MAX) {
    cpu_to_use = cmdline_opts.cpu_to_lock;
  }
...
int main(int argc, char** argv) {
  ...
  std::map<std::string, benchmark_func_t> str_to_func = DeclareBenchmarks();
