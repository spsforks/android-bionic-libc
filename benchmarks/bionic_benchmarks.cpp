...
void LockAndRun(benchmark::State& state, benchmark_func_t func_to_bench, int cpu_to_lock) {
  if (cpu_to_lock != INT_MAX) LockToCPU(cpu_to_lock);
...
int RegisterXmlBenchmarks(bench_opts_t cmdline_opts,
  ...
  while (fn) {
    ...
    run_args = ResolveArgs(run_args, xml_args ? android::base::Trim(xml_args->GetText()) : std::string());
...
int main(int argc, char** argv) {
  ...
  if (!opts.xmlpath.empty()) {
    if (int err = RegisterXmlBenchmarks(opts, g_str_to_func)) {
      return err;
    }
  }
  RegisterCliBenchmarks(opts, g_str_to_func);
