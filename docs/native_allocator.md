...
This document describes how to verify a new native memory allocator. This
procedure should be followed when upgrading or moving to a new allocator.
A small minor upgrade might not be necessary, however, at least the trace
benchmarks should always be executed because allocators can change in
unexpected ways.

## Android Extensions
Android supports a few non-standard mallopt controls that any  allocator
needs to support.

### Mallopt Extensions
These are mallopt options that Android requires to work efficiently.
...
This option is set by default for all applications on Android since it
doesn't require every free to make extra calls.
...
and no other allocation calls are made until that occurs. The below option
is used to handle this case.

#### M\_PURGE
When called, `mallopt(M_PURGE, 0)`, an allocator should purge and release
any unused memory immediately. If possible, this should include thread cached
memory if it exists. The idea is that this can be called to purge memory
that hasn't has not been purged when `M_DECAY_TIME` is set to one. This
is useful if you have a server application that does a lot of native
allocations and the application wants to purge that memory before
...
### Bionic Tests
The bionic unit tests contain a number of tests to verify that
an allocator is consistent with what Android expects.
...
Also, run the entirety of the bionic unit tests since the allocation tests
are only designed to verify behavior expected of an allocator by Android.
The allocation tests are not meant to be complete, so it is also expected
that a native allocator will have its own set of tests that will be run
on device.
...
This will run it against a target device, if there are multiple devices
connected to the system, the use the `-s <SERIAL>` to specify the device
to use.

## Performance
There are multiple different ways to evaluate performance on Android of
a native allocator. One is allocation speed in various different scenarios,
the other is total PSS taken by the allocator.
...
and then freeing it right away.
...
The last value in the display is the size of the allocation in bytes. It is
...
single thread numbers from trace data are actually representative of a
real world situation, and should be considered of higher priority.
...
The first set of these benchmarks does a set number of 8192 byte allocation
...
For these benchmarks, the last parameter is the total number of allocation to
do in each loop.

The other variation of this benchmark is to always do forty allocations in
each loop, but vary the size of the allocations. As with the other benchmark,
only the time it takes to do the allocations is tracked, the frees are
not counted. Forty allocations is an arbitrary number that could be modified
in the future. It was chosen because a version of the native allocator,
jemalloc, showed a problem at forty allocations.
...
For these benchmarks, the last parameter is the size of the allocation in
bytes.
...
a switch. The trace benchmarks are important since they simulate real
world allocation profiles.

#### SQL Allocation Trace Benchmark
This benchmarks is a trace of the allocations performed when running
the SQLite BenchMark app.

This benchmarks is designed to verify that the allocator will be performant
in a real world allocation scenario. SQL operations was chosen as a
...
this is a worst case scenario since it collapses all of the allocation
operations to occur one after another. This will cause a lot of threads
allocating at the same time. The trace data does not include timestamps,
...
the option [record\_allocs](https://android.googlesource.com/platform/bionic/+/master/libc/malloc_debug/README.md#record_allocs_total_entries) is used.
...
Every 1000 allocations, a dump of the PSS and VA space will be performed.
At the end, a final PSS and VA space number will be displayed. For the most
part, the intermediate data can be ignored, but it is always a good idea
to look over the data to verify that no strange spikes are occurring.
...
current allocator. They should be in the same range though.
...
and some allocators might leave large virtual address space maps around
rather than delete them. This has lead to allocation failures, so it is
important to verify that when running this trace using the 32 bit replay
the virtual address space consumed is not much larger than the current
allocator.

#### Performance Trace Benchmarks
This is a benchmark that treats the trace data as if all allocations
occurred in a single thread. This is the worst case scenario that could
happen if all of the allocations are spaced out in time.
...
traces and display data. It takes many minutes to complete these runs.

In order to create new traces see malloc_debug/README.md#record_allocs.


Mention svelte config.
