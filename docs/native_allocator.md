...
at least the trace benchmarks described
[here]({#SQL-Allocation-Trace-Benchmark}),
[here]({#Memory-Replay-Benchmarks}) and
[here]({#Performance-Trace-Benchmarks}) should be run.
...
## Android Extensions
Android supports a few non-standard mallopt controls that an allocator
needs to support.
...
tests for these in `bionic/tests/malloc_itearte_test.cpp`.
...
and no other allocation calls are made until that occurs. The `M_PURGE`
option is used to handle this case.
...
### Bionic Tests
The bionic unit tests contain a small number of the allocator tests. These
...
single thread numbers from trace data is actually representative of a
...
current allocator. They should be in the same range though as the current
values.
...
allocator. A small increase (on the order of a few MBs) would be okay.
