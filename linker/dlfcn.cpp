...
void* __android_dlopen_ext(const char* filename, int flags, const android_dlextinfo* extinfo) {
...
void* __dlvsym(void* handle, const char* symbol, const char* version) {
  void* caller_addr = __builtin_return_address(0);
...
soinfo* get_libdl_info() {
  if (__libdl_info == nullptr) {
    ...
    // we need this to be a part of global group (for now)
    __libdl_info->set_dt_flags_1(DF_1_GLOBAL);
