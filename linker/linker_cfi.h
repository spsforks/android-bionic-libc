...
class CFIShadowWriter : private CFIShadow {
  ...
  // Temporary storage location for the shadow start address.
  // Only used during initial link.
  uintptr_t shadow_start_tmp;

  // Pointer to the shadow start address. Initially points to shadow_start_tmp, later - to a
  // write-protected location in libdl.so.
  uintptr_t *shadow_start;

  bool initial_link_done;

 public:
  CFIShadowWriter() : shadow_start(&shadow_start_tmp) {}
