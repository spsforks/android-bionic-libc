...
TEST(linked_list, simple) {
  ...
  test_list_t list;
  ASSERT_STREQ("", test_list_to_string(list));
  ASSERT_TRUE(!alloc_called);
  ASSERT_TRUE(!free_called);
  list.push_front("a");
  ASSERT_TRUE(alloc_called);
  ASSERT_TRUE(!free_called);
  ASSERT_STREQ("a", test_list_to_string(list));
  list.push_front("b");
  ASSERT_STREQ("ba", test_list_to_string(list));
  list.push_front("c");
  list.push_front("d");
  ASSERT_STREQ("dcba", test_list_to_string(list));
  ASSERT_TRUE(alloc_called);
  ASSERT_TRUE(!free_called);
  alloc_called = free_called = false;
  ...
  ASSERT_STREQ("dba", test_list_to_string(list));
  alloc_called = free_called = false;
  list.remove_if([] (const char* c) {
    return *c == '2';
  });
...
TEST(linked_list, push_pop) {
  ...
  list.push_front("b");
  list.push_front("a");
  ASSERT_STREQ("ab", test_list_to_string(list));
  list.push_back("c");
  ASSERT_STREQ("abc", test_list_to_string(list));
  ASSERT_STREQ("a", list.pop_front());
  ASSERT_STREQ("bc", test_list_to_string(list));
  ASSERT_STREQ("b", list.pop_front());
  ASSERT_STREQ("c", test_list_to_string(list));
  ASSERT_STREQ("c", list.pop_front());
  ASSERT_STREQ("", test_list_to_string(list));
  ASSERT_TRUE(list.pop_front() == nullptr);
  list.push_back("r");
  ASSERT_STREQ("r", test_list_to_string(list));
...
TEST(linked_list, remove_if_then_pop) {
  ...
  ASSERT_STREQ("ad", test_list_to_string(list));
  ASSERT_STREQ("a", list.pop_front());
  ASSERT_STREQ("d", test_list_to_string(list));
