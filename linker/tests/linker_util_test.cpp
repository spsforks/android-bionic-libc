...
TEST(linker_util, parse_so_path_basic) {
    ...
    const char* input = "$ORIGIN/dir1:${ORIGIN}/dir2:${ORIGIN/dir3:/dir4:$LIB/dir5$:/data/libs/arch/$PLATFORM/libanswer.so";
    const char* expected[] = {
        "ORIGIN_PATH/dir1",
        "ORIGIN_PATH/dir2",
        "${ORIGIN/dir3",
        "/dir4",
        "/system/lib/dir5$",
        "/data/libs/arch/deep_thought/libanswer.so",
        NULL,
    };
    char output[1280];
    ...
    ASSERT_TRUE(parse_so_paths(input, array, output, 512, 10, origin_path, "/system/lib"));
    ...
    ASSERT_STREQ(expected[5], array[5]);
}
...
TEST(linker_util, parse_so_path_replace_token_long) {
    ...
    ASSERT_FALSE(parse_so_path_replace_token(output, 46, input, origin_path, "/system/lib"));
