...
overflow:
    ERROR("aprof : overflow @ %s", si->name);
    return;
}
...
static int aprof_alloc_tos(soinfo *si)
{
    ...
    void *addr;
    DEBUG("aproc : alloc tos for %s", si->name);
...
#error "Aprof not supported"
     ...
     */
    if (pc >= LINKER_TEXT_BASE &&
        pc <= LINKER_TEXT_BASE + LINKER_AREA_SIZE) {
        return;
    }
    ...
    /*
     * Skip non-profiling so under aprof jni mode
     */
...
static int aprof_open_file(const char *filename)
{
    ...
    size_t len_progname = strlen(filename);
...
static void aprof_write()
{
    int fd = aprof_open_file(somain->name);
    soinfo *si;
    uint32_t header_type;
...
static void aprof_write_tos(int fd, soinfo *si)
{
    ...
    for (fromindex = 0; fromindex < endfrom; fromindex++) {
        ...
        for (toindex = si->froms[fromindex]; toindex != 0;
            ...
            DEBUG("%x->%x : %d", frompc, selfpc, count);
