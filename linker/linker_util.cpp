 ...
 */
bool parse_so_path_replace_token(char* path, size_t path_size, const char* path_with_tokens, const char* origin_path, int parse_flags = PARSE_LDTOKEN_ALL) {
    if (path == NULL) {
        return false;
    }
    ...
    while( (token_start = strchr(src, '$')) != NULL ) {
        ...
        if (token != NULL && replacement != NULL && strstr(name_start, token) == name_start
...
bool parse_so_paths(const char* so_path, const char** array, char* buf, size_t buf_size, size_t max_count, const char* origin_path) {
    if(so_path == NULL) {
        return false;
    }
